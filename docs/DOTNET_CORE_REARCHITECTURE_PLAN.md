...
> **Status:** Pre-release baseline (`pre-release/1.0.0`) — Design Review v1.4 (post-remediation; supersedes v1.3)
...
## 3.33 v1.3 / v1.4 Review Remediation Addendum

This addendum closes the residual issues raised in the v1.3 self-review of §3.32 plus the v1.4 self-review of §3.33's own scaffolding. v1.3 introduced the design intent; v1.4 inline-corrects the v1.3 scaffolding gaps that emerged from a follow-up read (privilege assumptions, build-key possession, peer-credential portability, bounded-resource gaps). It supersedes v1.2 sign-off. Where this addendum disagrees with §3.32, **this addendum wins** until the parent section is folded in. The addendum is normative for v1 GA.

*Edit history:* the v1.3 baseline was committed as `8616780`; the v1.4 inline corrections (marked **v1.4** in their rule prefixes) replace the impossible/insecure v1.3 phrasings rather than appending a separate §3.34. This keeps the addendum readable as a single coherent whole.

### 3.33.1 Critical (block-GA)

#### 3.33.1.1 Audit-log trust root survives daemon self-update (amends KEY-ROT-1 ↔ §3.28.3 / UPD-RB-*) — **CR-1 (v1.3, corrected v1.4)**
...
**v1.4 correction:** the v1.3 scaffolding required the *outgoing daemon binary* to sign `BuildKeyRollover` records with a build-key whose *private* half lives in the build infrastructure's HSM and is never shipped to end-user binaries. Daemons can verify build-key signatures but cannot produce them. v1.4 decouples the two roles: the daemon never signs with a build-key. Continuity is delivered by the build pipeline producing a pre-signed release manifest at release time, and the daemon observing/recording the manifest under its own audit key.
...
| TRUST-ROOT-2 (v1.4) | The install anchor is itself authenticated by a **release manifest** signed by the offline-root key (TRUST-ROOT-3). At first start the daemon verifies the running binary's hash appears in a release manifest signed by the burned-in offline-root fingerprint (or its emergency-root pivot, see §3.33.4.3); only then does it generate the install anchor and stamp `installAuditAnchor.signedAtBuildHash = <runningBinaryHash>` so the chain can later be verified against the manifest. The daemon **never** signs with a build/code-signing key — daemons verify, the build pipeline signs. |
| TRUST-ROOT-3 | The offline-root key signs `release-manifest.signed.json` (per release) and `build-keys.signed.json` (per release-key rotation), both published at `https://aka.ms/aio-build-keys.json` (and pinned mirror). The release manifest enumerates `{ binaryHash, version, buildKeyFingerprint, supersedesBinaryHashes: [...], publishedAtUtc }` for every shipped artifact. The daemon refreshes the manifest on every `daemon update` and on a 24 h schedule; offline operation falls back to the cached manifest with a `BuildKeyManifestStale` event after `BuildKeys:StaleAfterDays` (default 30). |
| TRUST-ROOT-4 (v1.4) | Daemon self-update emits a `BuildKeyRolloverObserved` audit record signed by the **outgoing daemon's audit key** (which it possesses) carrying `{ outgoingBinaryHash, incomingBinaryHash, releaseManifestHash, releaseManifestSignatureValid: true }`. The incoming daemon, on first start, emits `BuildKeyRolloverContinuation` signed by its own audit key carrying the same triple; the verifier walks: audit-key chain → KeyTransition records (KEY-ROT-2, audit-key continuity) → BuildKeyRolloverObserved records (binary-hash continuity) → release manifests → offline-root. Every link in the chain is signed by a key the signer actually possesses. |
| TRUST-ROOT-5 | A binary whose hash is not in any verified release manifest is rejected at install/update time with `BuildKeyUntrusted { hash, manifestUrl }`; the operator must explicitly run `ai-orchestrator daemon trust-build-key --hash <hex> --confirm` to add it (audit-logged via `BuildKeyManuallyTrusted`, requires interactive confirmation). The override does not bypass the offline-root signature requirement on subsequent updates — it adds a single binary hash to a local trust list scoped to that install. |
| TRUST-ROOT-6 | Verifier semantics: `audit verify` walks segment chains back to the install anchor, then verifies the install anchor's `signedAtBuildHash` appears in the cached/fetched release manifest, then verifies the release manifest signature against the burned-in offline-root (or emergency-root). Any break is fail-closed and named (`AuditChainBrokenAtInstallAnchor`, `AuditChainBrokenAtReleaseManifest`, `AuditChainBrokenAtOfflineRoot`) so operators can act on the specific failure mode. |
| TRUST-ROOT-7 | The acceptance test `AuditChain_SurvivesUpdate_NoFalseTamper` runs `daemon update` 10 times across two synthetic build-keys, emits 10⁴ audit records spanning each transition, and asserts the verifier reports zero false-tamper events. A second test `AuditChain_RejectsUnsignedBinary` confirms an artifact whose hash is absent from the release manifest is rejected at update time. |

#### 3.33.1.2 Hook-gate nonce: deny hard-link theft (amends HK-GATE-NONCE-6..9) — **CR-2 (v1.3, corrected v1.4)**
...
**v1.4 correction:** v1.3's HK-GATE-LINK-1 required `chattr +i` (Linux `CAP_LINUX_IMMUTABLE`, root-only), `chflags uimmutable` (macOS, silently no-op'd in many container/network configurations), or DACL deny-link (Windows, where `CreateHardLinkW`'s permission model is brittle and deny-ACEs break inheritance). An unprivileged daemon cannot set these flags reliably. v1.4 promotes HK-GATE-LINK-3 (daemon-private-dir + bind-mount/junction) to the **primary** defense — it works without privilege — and demotes immutability flags to documented best-effort. The contract states the residual exposure honestly.

| # | Rule (HK-GATE-LINK-*) |
|---|---|
| HK-GATE-LINK-1 (v1.4) | **Primary defense:** nonce files live in a per-job daemon-owned directory `<dataDir>/hook-gate/<plan>/<job>/nonce` with `0700` (POSIX) / owner-only DACL (Windows) on the directory itself. The directory is exposed inside the worktree at `<cwd>/.github/hooks/.gate-nonce` via a daemon-set-up **bind-mount** (Linux, when CAP_SYS_ADMIN is held by the daemon's setup helper or the install configured an unprivileged user-namespace bind path) / **directory junction** (Windows, owner-only DACL on the junction itself) / **read-only mount** (where supported). The worktree-resident process sees a read-only view; the source file in `<dataDir>` is not in the worktree's mount namespace and cannot be hard-linked from worktree paths. Acceptance test asserts `link(<cwd>/.gate-nonce, <cwd>/stolen)` fails on bind-mounted setups across ext4/xfs/btrfs/ntfs. |
| HK-GATE-LINK-2 (v1.4) | **Secondary defense (always on):** on every nonce read, the daemon calls `fstat` on the opened handle and asserts `st_nlink == 1` (POSIX) / `NumberOfLinks == 1` from `BY_HANDLE_FILE_INFORMATION` (Windows). `st_nlink > 1` causes `deny` with reason `nonce-hardlinked` and emits `HookGateNonceShared { jobId, hardLinkCount }` (Security audit). This catches the case where bind-mount setup failed silently and still hard-links were created. |
| HK-GATE-LINK-3 (v1.4) | **Best-effort defense (where supported):** the daemon attempts `chattr +i` (Linux when CAP_LINUX_IMMUTABLE is held), `chflags uimmutable` (macOS), or DACL `DENY DELETE \| WRITE_DAC \| WRITE_OWNER` for non-owner principals (Windows). Successful application emits `HookGateNonceImmutabilitySet`; silent no-op or permission-denied emits `HookGateNonceImmutabilityUnsupported { reason }` and the operator is not surprised when the contract reduces to HK-GATE-LINK-1 + HK-GATE-LINK-2. Immutability is **not** assumed by any other rule. |
...
| HK-GATE-LINK-5 (v1.4) | **Honest residual contract:** on filesystems where bind-mount is unavailable (macOS, locked-down enterprise mount config, Windows where junction creation requires elevation that isn't held), the contract reduces to HK-GATE-LINK-2 + HK-GATE-LINK-4: an attacker with read access to the worktree can extract the nonce and replay within ≤ 5 minutes; rotation bounds the exposure but does not eliminate it. The doc surfaces this explicitly. The acceptance test `HookGate_HardLinkTheft_Defeated` is parameterized: on ext4/xfs/btrfs/ntfs with bind-mount it asserts the link operation itself fails; on apfs/fuse-overlayfs without bind-mount it asserts that *if* the read succeeds via the link, the subsequent gate evaluation is denied with `nonce-hardlinked` *and* the rotation interval bounds the replay window. |
...
| CONC-BROKER-2 (v1.4) | Per-user daemons talk to the broker via a **path-based** authenticated transport with filesystem permissions as the first auth gate: Windows named pipe `\\\\.\\pipe\\ai-orchestrator-coord` with PipeSecurity granting `READ_DATA \| WRITE_DATA` to `Authenticated Users`; POSIX `/run/ai-orchestrator/coord.sock` (or `$XDG_RUNTIME_DIR/ai-orchestrator/coord.sock` on systems without `/run`), parent dir mode `0755` and socket mode `0666` (the broker enforces per-user isolation in software, not via socket permissions). Abstract-namespace sockets are **explicitly rejected** because they bypass filesystem permissions and are reachable from any process in the same network namespace. The broker authenticates each connection via `SO_PEERCRED.uid` (POSIX) or `GetNamedPipeClientProcessId` → token SID (Windows) **and** authenticates each individual request via `SCM_CREDENTIALS` per `sendmsg`/`recvmsg` (POSIX) or `ImpersonateNamedPipeClient` per call followed by `RevertToSelf` (Windows) — so pid-recycling between connect and request is caught. The recovered identity scopes every reservation to that user. |
| CONC-BROKER-3 (v1.4) | The broker enforces per-user *and* per-host caps with **proportional fairness**: `Concurrency:PerUserMax` (default 8) is each user's slice; `Concurrency:PerHostMax` (default 16, configurable up by the host admin) is the hard ceiling. When the host cap is reached, the broker maintains a per-user FIFO admission queue and admits the next request from the user with the smallest current footprint, breaking ties by request arrival time. This bounds wait time for any user to `O(otherUsersWithJobs × theirJobAvgDuration / hostCap)`. A queued request surfaces an `actionableHint: "host capacity reached; queued at position N of M; estimated wait <duration>"` and emits `ConcurrencyHostQueued { userId, position, queueLength, estimatedWaitMs }` so dashboards can surface contention. Operators tune fairness via `Concurrency:HostFairness: Strict \| Proportional \| FirstComeFirstServed` (default `Proportional`). |
...
| # | Rule (CRED-IPC-*) |
|---|---|
| CRED-IPC-1 (v1.4) | The daemon spawns a per-helper-invocation authenticated transport that survives the spawn-into-git-into-helper hop, using **path-based** sockets/pipes (never abstract sockets, which bypass filesystem permissions and are reachable from any process in the same network namespace): Linux/macOS use a Unix socket at `/run/user/<uid>/ai-orchestrator/cred-<32-byte-random-hex>.sock` (parent dir `0700` owned by the daemon's uid, socket mode `0600`, unlinked on close); Windows uses a per-invocation named pipe `\\\\.\\pipe\\ai-orchestrator-cred-<random>` with PipeSecurity restricting to the spawning user's SID. Where `/run/user/<uid>` is unavailable (e.g., daemon not running under a session), `$XDG_RUNTIME_DIR` or `$TMPDIR/ai-orchestrator-cred-<uid>/` (created `0700`) is the fallback. |
| CRED-IPC-2 (v1.4) | The transport endpoint path is delivered via env var `AI_ORCHESTRATOR_CRED_PIPE=<path>` set on the spawned `git` process; git inherits the env to the credential helper. The helper opens the path; the daemon authenticates the peer via `SO_PEERCRED.uid` (POSIX) / `OpenProcessToken(GetNamedPipeClientProcessId).SID` (Windows) and asserts the connecting uid/SID equals the daemon's own — cross-user credential leakage is the threat model in scope. **Same-uid process isolation is explicitly out of scope**: a same-uid attacker can already `ptrace`/`ReadProcessMemory` the helper directly, so parent-pid walks (which are racy on POSIX, restricted on macOS without entitlements, and meaningless under same-uid threat) add no security and are not performed. The doc states this threat-model boundary explicitly: "CRED-IPC defends against cross-user credential leakage; same-uid process isolation is the responsibility of the host's process-isolation primitives (sandbox-exec, AppContainer, user-namespace, etc.)." |
| CRED-IPC-3 (v1.4) | The transport accepts the **full git credential-helper protocol verb sequence** (`get`, then `store` or `erase` from git's follow-up call), then tears down. The endpoint's lifetime is bounded by either (a) `store`/`erase` received, or (b) `Cred:HelperEndpointTimeoutMs` (default 30 s), whichever first. Verbs outside the expected sequence cause `protocol-violation` and immediate teardown. The endpoint name is unguessable (32 bytes from `RandomNumberGenerator.GetBytes`); even if the env var is leaked, the endpoint is gone after the verb sequence completes. A `CredentialIpcReusedAttempt { jobId, peerPid }` (Security audit) is emitted on any post-teardown connection attempt. |
...
| T2-READ-11 (v1.4) | **Bounded reassembly:** each reader maintains a reassembly buffer keyed by `recordSeq` carrying not-yet-complete chunk sets. The buffer is bounded by `T2:ReassemblyBufferMaxBytes` (default 16 MiB) and `T2:ReassemblyTimeoutMs` (default 5_000 ms) per reader. Overflow on either dimension drops the oldest incomplete record set, emits `EventLogReassemblyAbandoned { recordSeq, partialChunks, reason: "buffer-full" \| "timeout" }`, and surfaces a `SubscriberLagged`-equivalent event so subscribers can decide to resubscribe with a fresh `fromSeq`. Chunk headers carry a `recordSeq` that is **monotonic per writer**, so readers detect skipped records explicitly via gap-in-sequence rather than silently dropping. |
...
#### 3.33.3.1 CRED-INVAL-3 freeze becomes rate-limit + escalating backoff (amends CRED-INVAL-3) — **mn-1 (v1.3, corrected v1.4)**

The 5-minute hard freeze after two 401s blocks users who *just rotated* their PAT and now have a valid one. The v1.3 60 s rate-limit fixed the lockout but did not bound the *total* retry rate over time — a scripted loop could still issue ~1 retry/minute for hours, hitting the auth provider's per-account rate limits and risking provider-side temporary blocks.

v1.4 contract: subsequent retries against the same `(remoteUrl, account)` are allowed at most once per `Auth:RetryRateLimitSeconds` (default 60). After `Auth:MaxRetriesBeforeBackoff` (default 5) consecutive failures within `Auth:BackoffWindowMinutes` (default 60), the rate-limit escalates with exponential backoff (1 min → 2 min → 4 min → … → capped at 1 hour) until either a successful auth resets the counter to zero or the backoff window expires. Each retry surfaces `actionableHint: "credentials revoked or scope insufficient — verify provider settings"` plus a per-retry suggestion ("if you just rotated your token, allow up to 60s for the new one to propagate"); during backoff escalation the hint is augmented with `"backoff escalated to <duration> after <count> consecutive failures — next attempt allowed at <utc>"`. Cache invalidates immediately on success.
...
> Every daemon self-update emits **two** chained transition records, both signed by keys the signers actually possess (per v1.4 corrections to TRUST-ROOT-*): (1) a `KeyTransition` audit record per KEY-ROT-2 with `reason: "update-rollover"`, signed by both the outgoing and incoming **audit keys** (which the daemons hold); (2) a `BuildKeyRolloverObserved` audit record per TRUST-ROOT-4 signed by the outgoing daemon's **audit key**, recording the binary-hash transition and the verified release-manifest hash, plus a follow-on `BuildKeyRolloverContinuation` from the incoming daemon's audit key. Both records are written before the binary swap completes (UPD-RB-2's canary phase verifies both). The verifier walks **both** chains and reports independent failure modes (`AuditChainBrokenAtKeyTransition` vs `AuditChainBrokenAtReleaseManifest`). Update rollback (UPD-RB-3) reverses both transitions atomically: the audit chain regrows under the prior audit key, and the binary-hash chain reverts to the prior binary's hash; both reversals are themselves audit-recorded as `KeyTransition.reason: "update-rollback"` and `BuildKeyRolloverObserved.direction: "rollback"`. The verifier handles rollback by walking forward through both reversal records and continuing the chain post-rollback.

#### 3.33.4.3 Offline-root key governance (new in v1.4) — **XC-3 (v1.4)**

v1.3 introduced three offline-root-signed manifests (`build-keys.signed.json` / `release-manifest.signed.json` per TRUST-ROOT-3, `aio-skew-matrix.json` per SKEW-MFST-2, `aio-diagnose-keys.json` per DIAG-RECIP-2) all rooted in a single offline key. Concentrating trust in one key without a rotation/compromise story is a single-point-of-compromise: a leak of the offline root collapses every install's trust foundation. v1.4 adds an explicit governance contract.

| # | Rule (OFFLINE-ROOT-*) |
|---|---|
| OFFLINE-ROOT-1 | The offline-root keypair is generated in an air-gapped HSM under an **M-of-N shareholder ceremony** (default 3-of-5; documented in `docs/security/key-ceremony.md`). Signing operations require quorum re-assembly. Shareholders are project maintainers + an external trustee; rotation of shareholders is itself an M-of-N operation. |
| OFFLINE-ROOT-2 | **Two roots are burned into the daemon binary** at build time: `currentOfflineRootFingerprint` (the active signer) and `emergencyOfflineRootFingerprint` (the recovery signer). The daemon trusts manifests signed by either root; under normal operation only `current` signs. The emergency root's private key lives in a separate HSM with a separate shareholder set, never co-located with the current root, and is used only for compromise-recovery. |
| OFFLINE-ROOT-3 | **Scheduled rotation** of the current root every `OfflineRoot:ScheduledRotationYears` (default 5). A rotation ceremony produces a `RootTransition` manifest signed by **both** the outgoing current root and the incoming current root, published at the same `aka.ms/aio-build-keys.json` URL. The daemon refreshes on update + 24 h schedule, verifies the transition, and updates its cached `currentOfflineRootFingerprint` (the burned-in fingerprint becomes the *minimum* trusted root — the cache walks forward via `RootTransition` chains from the burned-in value). |
| OFFLINE-ROOT-4 | **Compromise-recovery** is signed by the emergency root: an `EmergencyRevocation` manifest naming the compromised current-root fingerprint, the new current-root fingerprint, the `compromisedAfterUtc` timestamp, and a freeform `incidentReference`. The daemon verifies the manifest against the burned-in `emergencyOfflineRootFingerprint`, marks every prior signature by the compromised root as `trusted-historical-only` (allowed for verifying past audit chains, never for verifying new manifests), and pivots to the new current root. The emergency root then signs a **single** subsequent `RootTransition` re-establishing a fresh current root before retiring back to standby — the emergency root is never used as the day-to-day signer. |
| OFFLINE-ROOT-5 | **Compromised-historical handling:** audit segments that were signed via a chain anchored on a now-compromised root remain verifiable for forensic purposes (the chain is mathematically unbroken) but `audit verify --strict` fails them with `AuditChainAnchoredInCompromisedRoot { rootFingerprint, compromisedAfterUtc }`. Default verification (non-strict) reports a warning but accepts the chain, since post-compromise tampering of pre-compromise data is provable to be earlier than the compromise window. |
| OFFLINE-ROOT-6 | **Acceptance test** `OfflineRoot_EmergencyRotation_Pivots` runs a synthetic compromise scenario: emits an `EmergencyRevocation` manifest, asserts the daemon refuses new manifests signed by the compromised root, accepts manifests signed by the new current root, and reports historical-chain warnings under `--strict`. A second test `OfflineRoot_ScheduledRotation_Continuity` runs a normal rotation and asserts no false-tamper events on chains spanning the rotation. |
| OFFLINE-ROOT-7 | **Public transparency:** every signing operation against the current or emergency root is published to a Sigstore-style transparency log (`https://aka.ms/aio-trust-log`) within 24 hours; the log itself is append-only and witnessed by independent observers. The daemon's manifest-refresh path verifies the manifest's transparency-log inclusion proof; manifests not in the log are rejected with `ManifestNotInTransparencyLog`. |

### 3.33.5 Sign-off & verification

Per §0, every change in this addendum was reviewed under the same three lenses as v1.1/v1.2:

- **Architecture:** signed off on TRUST-ROOT-* as v1.4-corrected (per-install anchor + offline-root release-manifest verification, no impossible build-key signing by daemons), HOST-SCOPE-* (in-proc client renaming and ALC risk elimination), CONC-BROKER-* with v1.4 path-based-socket + proportional-fairness corrections, EV-GEN-3 publisher-anchored deadlines, and OFFLINE-ROOT-* governance.
- **Security (AssumeBreach):** signed off on HK-GATE-LINK-* as v1.4-corrected (bind-mount-primary, immutability-best-effort, residual exposure stated honestly), CRED-IPC-* as v1.4-corrected (path-based sockets, per-message SCM_CREDENTIALS, threat model bounded to cross-user), TRUST-ROOT-* (chain survives self-update without impossible signing requirements), DIAG-RECIP-* (rotation + per-tenant), CONC-BROKER-* (no world-writable shared state, no abstract sockets), PATH-VAL POSIX `openat(O_NOFOLLOW)` walk (TOCTOU-free), T3-RED entropy heuristic (denylist-only → redact-by-default), and OFFLINE-ROOT-* (M-of-N ceremony, dual-root burn-in, transparency log).
- **Reliability:** signed off on T2-READ-6..11 (filesystem-aware split-record fallback with bounded reassembly buffer), CRED-INVAL-3 v1.4 escalating-backoff (rate-limit with exponential cap), mn-2 one-time config consent (operational ergonomics), LV-ASYNC-* (handlers can do real work without losing the zero-copy fast path), and CONC-BROKER-3 v1.4 proportional fairness (no starvation under host-cap saturation).

The same three-checkbox PR template applies. v1.4 supersedes v1.3; the §0 status header reflects the new baseline.

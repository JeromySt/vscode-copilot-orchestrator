...
public interface IAiOrchestrator
{
    ...
    // Node control
    Task<RetryNodeResult>     RetryNodeAsync             (RetryNodeRequest      request, CancellationToken ct);
    Task<ForceFailNodeResult> ForceFailNodeAsync         (ForceFailNodeRequest  request, CancellationToken ct);
    Task<NodeFailureContext>  GetNodeFailureContextAsync (NodeRef               node,    CancellationToken ct);
    Task<NodeLogsResult>      GetNodeLogsAsync           (NodeLogsRequest       request, CancellationToken ct);
...
public sealed record RetryNodeRequest
{
    public required PlanId  PlanId        { get; init; }
    public required NodeId  NodeId        { get; init; }
...
public sealed record NodeLogsRequest
{
    public required PlanId  PlanId  { get; init; }
    public required NodeId  NodeId  { get; init; }
...
/// </summary>
public sealed record NodeLogsResult
{
    public required string         FilePath  { get; init; } // absolute path under .aio/.ps/log/<planId>/<nodeId>/<phase>/<attempt>.log
...
| SR-2 | The captured id is returned in `AgentRunResult.SessionId`. The orchestrator persists it on the node attempt record. |
| SR-3 | On retry of the same node, the orchestrator passes the captured id back as `AgentRunRequest.SessionId`. The runner emits `--resume <id>` (A13). |
| SR-4 | Each node has its own `<configDir>` (A9), so concurrent jobs never collide on session storage. |
...
public enum ExitCodePolicy
{
    ...
    AnyExitIsSuccess,        // diagnostic-only nodes
...
public interface IPlanRepository
{
    ...
    Task<PlanInstance>  AddNodeAsync  (AddNodeRequest      request, CancellationToken ct);
...
{
    public required PlanId  PlanId { get; init; }
    public required JobSpec Node   { get; init; }
    public IReadOnlyDictionary<string, JsonElement> Extensions { get; init; }
        = ImmutableDictionary<string, JsonElement>.Empty;
}
```
...
public interface IWorktreeManager
{
    ...
    /// any artifacts that should not survive the node attempt. Throws if the worktree
...
public interface IAsyncWorktreeLease : IAsyncDisposable
{
    ...
    /// <summary>The detached branch ref for this attempt (`aio/<plan8>/<node8>/<attempt>`).</summary>
...
Node execution becomes a single try-scoped block where the language guarantees disposal:

```csharp
public async Task<NodeRunResult> ExecuteNodeAsync(NodeRunRequest request, CancellationToken ct)
{
...
The pattern composes naturally with the daemon's host-shutdown signal: `IHostApplicationLifetime.ApplicationStopping` cancels every active `NodeRunRequest`'s CT, which causes `RecordOutcome(Canceled)` + `DisposeAsync` to run for every live lease before the daemon process exits. This is the **clean-shutdown path**; the registry-based reclaim above handles only the **crash path** (process killed without unwind).

##### Diagnostics & events

Every transition publishes on the bus (§3.4.1 Git category, additive):

- `WorktreeLeaseAcquired { leaseId, planId, nodeId, attempt, worktreeRoot, baseCommit }`
...
public async Task Lease_Is_Disposed_When_Phase_Throws()
{
    ...
    var executor = new NodeExecutor(fake, ThrowingPhaseRunner.Instance);

    await Assert.ThrowsAsync<InvalidOperationException>(() => executor.ExecuteNodeAsync(req, CancellationToken.None));
...
| **Node lifecycle** | `NodeTransition` (with from/to/reason), `NodeReady`, `NodeStarted`, `NodeRetrying`, `NodeBlocked`, `NodeSucceeded`, `NodeFailed`, `NodeForceFailed`, `NodeCanceled` |
...
{
    public IReadOnlyCollection<string>? CategoryAllowList { get; init; } // e.g. {"Node", "Agent"}
...
```jsonc
{
  ...
  "totalEvents": 12345,
  "durationMs": 7200000,
  "nodeOutcomes": [ /* one entry per node: id, status, attempts, finalCommit */ ],
  "failureSignals": [ /* concise list of failed phases + first error line per failure */ ],
  "timeline": [ /* coarse phase boundaries: planStarted, firstNodeStarted, ... */ ]
}
```
...
| E11 | **Memory-mapped files for log reads ≥ 1 MiB.** `INodeLogReader.OpenAsync` returns either a `FileStream` (small files) or a `MemoryMappedViewAccessor` (large files), selected automatically by size. The UX layer can scroll through a 100 MiB log without ever loading more than one OS page at a time. | mmap is the only sane way to read multi-MiB logs without LOH pollution; it also lets multiple subscribers share the same physical pages. |
| E12 | **Periodic GC budget assertion in CI.** A long-running soak test (`AiOrchestrator.IntegrationTests.GcSoakTests`) executes a 50-node plan with scripted agents and asserts gen-2 collection count ≤ N and RSS growth ≤ M MiB over the run. PRs that regress the budget by > 10% are blocked. | Memory regressions are silent until they bite production. A CI gate makes them as visible as a failed test. |
...
flowchart LR
    ...
    J --> L[Node-binding dispatcher<br/>napi_threadsafe_function]
    K --> M[Client deserializer<br/>source-gen, pooled]
    M --> N[UX requests log slice by path+range<br/>NodeLogsResult descriptor]
...
`NodeLogsResult` (refactored in §3.1) is a **descriptor**. The daemon never reads the log file to satisfy a `GetNodeLogsAsync` call unless the caller explicitly asks for an inline slice with a bounded `ByteRange`. The intended consumption pattern:

```csharp
// 1. Get the descriptor (cheap; ~200 bytes over the wire)
var logs = await aiOrchestrator.GetNodeLogsAsync(new NodeLogsRequest { … }, ct);
...
For the cross-host case (Mode C centralized daemon, MCP client on a different machine, browser-based UX) the caller may request a bounded inline slice via `NodeLogsRequest.Range`. Slices are **capped at 64 KiB** (E2); larger reads must be chunked into multiple requests by the caller.
...
- **Identifiers** — `PlanId`, `NodeId`, `RunId`, branch names, producer ids. Bounded length, low cardinality, interned where it pays.
...
`string` is **not** allowed for: log content, stdout/stderr lines, instructions files, JSON payloads, file content of any kind, or any field whose size scales with plan / node / log volume.
...
3. **GC soak test** in `AiOrchestrator.IntegrationTests` (E12) runs a 50-node scripted plan and asserts gen-2 count and RSS growth ceilings.
...
`Microsoft.IO.RecyclableMemoryStream` **is** used — for the log-writer's own batched write buffer (one stream per node-attempt log file, holding ~256 KiB of pending writes), not for per-line dispatch.
...
Once a `RecyclableSegment` is committed by the LineSplitter, the log-writer queue (one per node-attempt) pulls it via the bus's standard bounded `Channel<RecyclableSegment>` and appends it to a per-attempt `RecyclableMemoryStream` buffer. The buffer is flushed to disk on whichever fires first:
...
- **Phase boundary**: `PhaseExited` event for the node forces a flush.
...
DaemonHostCts                                  (process shutdown)
  └── PlanCts(planId)                          (plan pause/cancel/archive)
        └── NodeRunCts(planId, nodeId, attempt)  (node retry / force-fail / orphan)
...
For each in-flight MCP tool invocation, the orchestrator's MCP server registers `(progressToken, NodeRunCts)`. When MCP's `notifications/cancelled { requestId, reason }` arrives, the server cancels the CTS and emits an outbound `notifications/cancelled` to the same client confirming the cancel reached its destination. Cancellations originating from the daemon (timeout, parent plan canceled) push a `notifications/cancelled` to the MCP client first, then close the request frame with a structured error.
...
public readonly record struct CancelScope(
    CancelScopeKind Kind, // Daemon | Plan | Node | Phase | Agent
    PlanId? PlanId = null,
    NodeId? NodeId = null,
    int?    Attempt = null,
...
Unit tests can drive cancellation at any level without spawning real processes; the cascade behavior is verified via the bus events emitted at each transition (`PlanCanceled`, `NodeCanceled`, `AgentExited { disposition: Killed, reason: <CancelReason> }`).
...
- `Node` then unambiguously refers to a *graph node* in the DAG-theory sense, which only matters in `IDagAnalyzer` / topology code.
...
|---|---|
| `NodeId` (entity) | `JobId` |
| `NodeRef` (entity ref) | `JobRef` |
| `RetryNodeRequest` / `RetryNodeResult` | `RetryJobRequest` / `RetryJobResult` |
| `ForceFailNodeRequest` | `ForceFailJobRequest` |
| `NodeFailureContext` | `JobFailureContext` |
| `NodeLogsRequest` / `NodeLogsResult` | `JobLogsRequest` / `JobLogsResult` |
| `NodeTransition` (event) | `JobTransition` |
| `NodeStarted` / `NodeSucceeded` / `NodeFailed` / `NodeCanceled` | `JobStarted` / `JobSucceeded` / `JobFailed` / `JobCanceled` |
| `NodeRunRequest` / `NodeRunResult` | `JobRunRequest` / `JobRunResult` |
| `IAsyncJobLease` (already named correctly above) | unchanged |
| `dagUtils.NodeId` (graph theory) | `DagNodeId` (kept distinct — graph node ≠ plan job in the abstract; the concrete plan-DAG happens to map 1:1) |

MCP wire shapes keep their existing tool names (`add_copilot_plan_job`, etc.) so older clients are unaffected; the **internal C# API uses Job everywhere**. Any earlier section in this document that says "node" in the context of "a job in a plan DAG" should be read as the `Job` rename when implementing.
...
A 10K-node plan with undetected cycles, or 100 subscribers pulling at very different rates, hangs the daemon today. The TS implementation has had subtle bugs in both areas.
...
`IDagAnalyzer.DetectCycles` runs during `Finalize` and after every `Reshape` operation. Implementation is iterative DFS (avoiding stack overflow on the maximum 1024-node graph). Cycles are reported as ordered node lists in `PlanCycleDetected { cycle: [jobIds...] }` so the user can act.
...
Completions are aware of dynamic state where useful: `plan resume <TAB>` enumerates paused plans; `node retry <plan> <TAB>` enumerates failed jobs. Dynamic completion calls the running daemon when one exists; when no daemon is running it falls back to static command-name completion only (no expensive cold start just for tab-complete).

### 3.23.2 Accessibility for UX surfaces

The VS Code panels (sidebar, plan detail, node detail, status bar) are bound by:
...
- The webviews handle keyboard navigation: arrow keys move focus through the DAG visualization, Enter activates a node, Esc closes panels.
...
public sealed record TsPlanInspection
{
    ...
    public required int RunningNodeCount { get; init; }                     // > 0 ⇒ requires special handling, see §3.25.4
...
| Branch refs `aio/p/<planId>/<nodeId>` (TS naming) | Same convention (preserved) | No-op; .NET inherited the branch-naming from TS deliberately. |
| Log files at `.aio/.ps/log/<planId>/<nodeId>.log` (single file per node) | `.aio/.ps/log/<planId>/<nodeId>/<phase>/<attempt>.log` (per-phase, per-attempt) | Conservative: copy the single TS log to `phase=work/attempt=1.log`, leave other phases empty. Annotate with a `LegacyLogLayout` finding. |
...
For a paused plan with `RunningNodeCount > 0` (a node was running when the plan was paused), the converter:

- Treats the running node as **failed with `Migration` reason**: emits a synthetic `NodeFailed { reason: "migration-from-ts", attempt: N }` event after the last TS-recorded event for that node.
...
- The user resumes the plan in the .NET daemon; the failed node enters the normal retry path.

This is the only correctness compromise the converter makes, and it is loud and recoverable: the user sees exactly one failed node per migrated-while-running plan, with full failure context pointing at the migration.
...
- Fixtures cover: pristine plan, single-job plan, fan-in plan, plan with reshape history, plan with failed node, paused plan, plan with running node at pause time, plan with corrupted `events.jsonl` last-line.
...
| Node | `node.retry`, `node.forceFail`, `node.failureContext`, `node.logs` | `IAiOrchestrator` |
...
1. When a daemon receives any **mutating** RPC for a `PlanId` (anything but `plan.list`, `plan.get`, `plan.graph`, `plan.export`, `events.subscribe`, `node.logs`, `node.failureContext`), it consults `PlanOwnership`.
...
| `node.logs` (descriptor only) | 2 ms | 6 ms | No file content over wire. |
...
It exists because a re-architecture that gets the supporting cast right but the DAG core wrong produces a re-architecture-of-the-re-architecture six months later. The Tier-1 invariants — node state machine, phase pipeline, persistence atomicity, scheduler readiness, bus replay, reshape, single-writer lease — are the deep ones; mistakes in any of them propagate to every front end. Tier-2 (auto-heal, quotas, cancel, failure context, recovery, cross-plan deps) and Tier-3 (metrics catalog, OTel schema, error code enum, etc.) build on Tier-1.

The section is organized so each gap from the readiness review has its own subsection:

| Tier | # | Subsection | Topic |
|---|---|---|---|
| 1 | 1 | §3.30.1 | Node state machine — exhaustive transitions |
...
### 3.30.1 Node state machine — exhaustive transitions

Every node lives in exactly one of nine states at any instant. Transitions are atomic with respect to the plan store (§3.30.3) and emit exactly one `NodeStateChanged` event (§3.30.5).
...
public enum NodeState
{
    ...
    Canceled     = 7, // User canceled the plan or this node specifically before terminal.
    ForceFailed  = 8, // Operator-issued `force_fail_copilot_job`; behaves like Failed for downstream propagation.
}
```

`Succeeded`, `Failed`, `Canceled`, `ForceFailed` are **terminal** states. `Blocked` is **derived-terminal**: it becomes the node's resting state once all its blocking ancestors are terminal, but it is reset to `Pending` if the blocking ancestor transitions away from a failure state via retry (§3.30.8).
...
|---|---|---|---|---|---|---|
| T1 | (none) | Pending | `add_job` / `scaffold` | Plan in scaffolding or running/paused, no cycle | Persist node record (§3.30.3) | `NodeAdded` |
| T2 | Pending | Ready | Scheduler readiness sweep (§3.30.4) | All deps in `Succeeded`; plan not paused | Set `readyAt` | `NodeReady` |
| T3 | Pending | Blocked | Readiness sweep | Any dep in `{Failed, ForceFailed, Canceled, Blocked}` | Set `blockedBy = [depId]` | `NodeBlocked` |
| T4 | Pending | Canceled | Plan cancel | Plan in `Canceling` | None beyond state | `NodeCanceled` |
| T5 | Ready | Scheduled | Scheduler dispatches | Capacity available (§3.30.4); worktree lease acquirable | Reserve worktree lease (§3.30.7); record `scheduledAt`, `attempt = current+1` | `NodeScheduled` |
| T6 | Ready | Pending | Reshape (T20) | Dep added/restored | Re-evaluate readiness | `NodeReshaped` |
| T7 | Ready | Canceled | Plan cancel | Plan in `Canceling` | Release any pre-reservation | `NodeCanceled` |
| T8 | Scheduled | Running | Phase pipeline starts | merge-fi phase entered | Record `startedAt`; create attempt directory (§3.30.3) | `NodeStarted` (`AttemptStarted`) |
| T9 | Scheduled | Failed | Worktree-lease acquisition failed terminally | Lease error not retryable | Release reservation; record failure context (§3.30.11) | `NodeFailed` |
| T10 | Scheduled | Canceled | Plan cancel before phase pipeline starts | — | Release reservation | `NodeCanceled` |
| T11 | Running | Succeeded | merge-ri phase completed | All phases passed | Set `succeededAt`; release lease | `NodeSucceeded` |
| T12 | Running | Failed | A phase failed and auto-heal declined or exhausted | See §3.30.8 | Persist failure context (§3.30.11); release lease | `NodeFailed` |
| T13 | Running | Running | Auto-heal retry within same attempt | Retry budget remains | Increment `phaseRetryCount` | `NodePhaseRetry` |
| T14 | Running | Canceled | Plan/node cancel during execution | — | Cascade cancel via §3.30.10 chain; release lease | `NodeCanceled` |
| T15 | Running | ForceFailed | Operator `force_fail_copilot_job` | Node currently `Running` or `Scheduled` (per §3.30 rule R-FF-1) | Stop process via §3.30.10; set `forceFailedBy` | `NodeForceFailed` |
| T16 | Failed | Ready | `retry_copilot_job` (full-attempt retry) | Plan running/paused; node has retry budget; user supplied or accepted same work spec | Increment `attempt`, clear `failureContext.current`, archive previous attempt (§3.30.11) | `NodeRetryRequested` |
| T17 | Failed | Pending | Reshape that adds a dep upstream | New dep is non-terminal | Re-evaluate readiness next sweep | `NodeReshaped` |
| T18 | ForceFailed | Ready | `retry_copilot_job` | Same as T16 | Same as T16 plus clear `forceFailedBy` | `NodeRetryRequested` |
| T19 | Blocked | Ready | A blocking ancestor transitioned Failed→Ready (T16) and now Succeeded | All deps now `Succeeded` | Same as T2 | `NodeReady` |
| T20 | Blocked | Pending | Reshape removed the blocking dep | New dep set non-terminal-failed | — | `NodeReshaped` |
| T21 | Blocked | Canceled | Plan cancel | — | — | `NodeCanceled` |
| T22 | Canceled | Ready | `retry_copilot_job` after cancel | Plan no longer in `Canceled`/`Canceling` | Same as T16 | `NodeRetryRequested` |
| T23 | Succeeded | Pending | Reshape that invalidates this node (e.g., dep added that re-runs upstream) | Explicit `reshape` op `invalidate_node` | Archive previous attempt; clear success record | `NodeInvalidated` |

**Forbidden transitions** are everything not in the table above. Examples: `Pending → Running` (must pass through `Ready` and `Scheduled`), `Succeeded → Failed` (terminal-success is sticky), `Running → Ready` without a phase failure (must go through `Failed` first), `Scheduled → Pending` (only via reshape). The state-machine module is single-entry: every transition flows through `INodeStateMachine.Transition(NodeId, TransitionRequest)` which validates against the table and rejects illegal transitions with `InvalidNodeTransitionException`.
...
- **Re-arming an attempt is *always* a new attempt**, never a reset of the existing one. T16/T18/T22 increment `attempt` (1-based, monotonic per node, never reused). All event subscribers see a fresh `AttemptStarted` with the new number; failure context for prior attempts is preserved (§3.30.11) and individually addressable.
- **Snapshot Validation (`__snapshot-validation__`) node** transitions follow the same table but its dependency set is auto-managed (§3.30.6 reshape rule R-RS-7), and its work spec is sourced from `Plan.VerifyRi` (§3.2.6 carries the original concept). It is never `force_failed` (analyzer `OE0042` rejects the operator action).
- **`expectsNoChanges = true` nodes** (§3.30.2 phase semantics) do not skip any transition — their commit phase succeeds-with-no-commit instead of failing, but the state-machine path is identical.

Analyzer **`OE0042`** validates `INodeStateMachine` impls: every transition method body must end with a single call to `EmitTransition(...)` and must reject any `(from, to)` pair not in the §3.30.1.2 table.

### 3.30.2 Phase pipeline — 8-phase normative contract

Every running node executes the same 8-phase pipeline. Phase boundaries are the only places where work-spec validation, persistence, retries, and event emission occur. The pipeline runs **once per attempt** (§3.30.1.4).
...
public enum ExecutionPhase
{
    ...
    MergeRi    = 7, // Reverse-integrate: merge this node's commit into the snapshot branch (§3.3)
...
| PH-3 | Prechecks | Setup succeeded; env in place | Execute `node.prechecks` work spec via the same pipeline as `work` (process / shell / agent) | Prechecks exit 0 | `attempts/<n>/phase/prechecks/log.bin`, `result.json` | Auto-heal: re-run once if work spec is process/shell and exit code in `[1,2]` (transient); else Fail | `node.prechecks == null` |
| PH-4 | Work | Prechecks passed (or skipped) | Execute `node.work` via the appropriate runner: `IAgentRunner` for agent specs, `IShellRunner` family for shell specs (§3.2.8), `IProcessLifecycle` direct for process specs | Work exit 0 (or `expectsNoChanges` semantics permit) | `attempts/<n>/phase/work/log.bin`, `agent-events.jsonl` (when agent), `result.json` | Auto-heal per §3.30.8 policy table (transient retry, agent retry-with-fresh-session, agent retry-with-resume) | Never |
| PH-5 | Commit | Work succeeded | `git add -A`; if changes present → `git commit -m "<auto>"` then `git push` to `attempts/<n>/branch`; if no changes → see ZeroChange rule | HEAD updated; remote ref pushed (or zero-change recorded) | `attempts/<n>/phase/commit/{commitSha,changedFiles[],zeroChange}.json` | Auto-heal: stash + retry-from-prechecks once on push race; else Fail. **ZeroChange rule:** if `node.expectsNoChanges == true`, record `zeroChange=true` and succeed; else fail with `NoChangesProduced` | Never |
| PH-6 | Postchecks | Commit succeeded | Execute `node.postchecks` work spec | Postchecks exit 0 | `attempts/<n>/phase/postchecks/log.bin`, `result.json` | Auto-heal: agent retry-from-postchecks (with the failure log fed back into agent context); shell/process: single re-run | `node.postchecks == null` |
| PH-7 | MergeRi | Postchecks passed | Merge this node's commit into the plan's snapshot branch (§3.3); cherry-pick if linear-history mode | Snapshot branch advanced; remote pushed | `attempts/<n>/phase/merge-ri/{snapshotSha,strategy}.json` | Auto-heal: snapshot lock contention (§3.30.7) → wait + retry once; merge conflict → escalate to operator; else Fail | Never |
| PH-8 | Finalize | MergeRi succeeded | Release worktree lease (§3.30.7); archive logs (§3.30.11 retention); emit `NodeSucceeded`; trigger downstream readiness sweep | Lease released; node terminal-success | `nodes/<id>/state.json` updated atomically | Cannot fail (all I/O is best-effort with bounded retries); failures here log but do NOT regress to `Failed` | Never |

#### 3.30.2.3 Phase invariants

| # | Invariant |
|---|---|
| PI-1 | Phases run **strictly sequentially per attempt**. No phase parallelism within a node. (Cross-node parallelism is §3.30.4.) |
...
| PI-5 | Auto-heal retries (T13) are bounded by `node.autoHeal.maxAttemptsPerPhase` (default 1 except agent work, which defaults to 2 — see §3.30.8). |
| PI-6 | A phase failure that exhausts auto-heal terminates the node attempt (T12). The next user retry (T16) starts a brand-new attempt at PH-1. There is no "resume from failed phase" within an existing attempt — new attempt always re-runs every phase from MergeFi. This is intentional: it ensures the worktree state is reproducible. |
...
│       ├── topology.json            # Node DAG: producerIds, deps, groups (ordering-stable serialization)
...
│       ├── nodes/
│       │   └── <nodeId>/
│       │       ├── node.json        # Producer ID, group, work spec, prechecks, postchecks, autoHeal flags
│       │       ├── state.json       # Current NodeState + scheduledAt/startedAt/succeededAt + currentAttempt
│       │       └── attempts/
│       │           └── <n>/         # 1-based, monotonic
...
│       └── snapshots/
│           ├── current.json         # Latest accepted snapshot (commit SHA + node-set)
│           └── archive/<rev>.json
...
| RW-6 | All filenames inside `.aio/.ps/` are ASCII-only and bounded to 80 chars to stay under Windows `MAX_PATH` even when the repo is deeply nested. The transformation `nodeId → filesystemSlug` is `Base32-Crockford(SHA256(nodeId))[..16]` (§3.15 long-path mitigation). |
...
The scheduler decides **which Ready node runs next**. It is the single owner of T2/T3/T5/T19 (state-machine readiness transitions). It is stateless across decisions — every decision is computed from `IPlanStore` snapshot + cross-plan capacity counters + per-plan paused state.
...
public interface IScheduler
{
    ...
    /// and dispatches up to N nodes (where N is bounded by global + per-plan capacity).
...
{
    public required SchedulerTrigger Trigger { get; init; } // Periodic | NodeStateChange | PlanResume | Reshape | CapacityFreed
    public PlanId? OnlyPlan { get; init; } // optional scoping for targeted sweeps
}

public sealed record SchedulerSweepResult
{
    public required IReadOnlyList<NodeId> NewlyReady   { get; init; }
    public required IReadOnlyList<NodeId> NewlyBlocked { get; init; }
    public required IReadOnlyList<NodeId> Dispatched   { get; init; }
...
| # | Rule |
|---|---|
| R-RD-1 | A node is `Ready`-eligible iff every dep is `Succeeded`. |
| R-RD-2 | A node is `Blocked`-eligible iff any dep is in `{Failed, ForceFailed, Canceled, Blocked}`. |
| R-RD-3 | Blocked-propagation is transitive but lazy: a `Pending` node does not become `Blocked` until the readiness sweep visits it AND finds the failure ancestor. (No background propagation thread; transitions only fire during sweeps.) |
| R-RD-4 | A node in `Pending` whose ancestor recovers (T16 retry) returns to readiness on the next sweep — there is no separate "unblock" command. |
| R-RD-5 | Readiness sweeps are triggered by: scheduler periodic timer (default 250 ms), any `NodeStateChanged` event, any `PlanResumed` event, any `ReshapeApplied` event, any `CapacityFreed` event (a node moved to terminal). The implementation deduplicates trigger storms within a 50 ms window. |

#### 3.30.4.3 Dispatch algorithm (R-DP-*)

| # | Rule |
|---|---|
| R-DP-1 | Across all plans, the global concurrent-job cap is `IGlobalCapacity.MaxConcurrentNodes` (default 8; configurable). The cap counts nodes in `Scheduled` ∪ `Running`. (Cross-references §3.9 for cross-daemon cap.) |
...
| R-DP-4 | Dispatch order across plans is **weighted-round-robin by plan submission time**: each plan gets one dispatch turn per round; plans with no Ready nodes skip. This prevents one large plan from starving others, and the deterministic ordering makes dispatch reproducible in tests. |
| R-DP-5 | Within a plan, Ready nodes dispatch in **deterministic order**: ascending `(group_path, producerId)` lexicographic. (No priority field in v1; group ordering gives users a reliable knob via group naming.) |
| R-DP-6 | A node fails to dispatch (back to `Ready`, no transition) iff its worktree lease cannot be acquired non-blockingly (§3.30.7). The scheduler logs `LeaseUnavailable { nodeId, leaseHeldBy }` and tries the next Ready node. The blocked node retries on the next sweep. |
| R-DP-7 | Plans in `Paused` state are excluded from dispatch entirely. Already-`Running` nodes in a paused plan are NOT canceled — they run to completion; only new dispatch is suspended. |
...
| FAIR-2 | No node can be starved indefinitely: dispatch order is deterministic (R-DP-5), so a Ready node either dispatches on its turn or its turn shifts forward when the head-of-queue node consumes a slot. |
...
| FAIR-4 | Reshape (§3.30.6) cannot starve in-flight nodes — reshape applies between sweeps, never preempting a Running node. |
...
| `NodeAdded`, `NodeRemoved`, `NodeReshaped`, `NodeInvalidated` | Topology changes |
| `NodeReady`, `NodeBlocked`, `NodeScheduled`, `NodeStarted`, `NodeSucceeded`, `NodeFailed`, `NodeForceFailed`, `NodeCanceled`, `NodeRetryRequested`, `NodePhaseRetry` | State machine (§3.30.1.2) |
...
Reshape mutates the DAG topology after `finalize_copilot_plan`. It is the highest-risk surface in the engine because it can introduce cycles, orphan running nodes, or leave Snapshot Validation pointing at the wrong leaf set.

#### 3.30.6.1 Reshape operations (R-OP-*)

| Op | Semantics |
|---|---|
| `add_job` | Insert a new node; deps point at existing producerIds |
| `remove_job` | Delete an existing node — only allowed if node is in `Pending` ∪ `Ready` ∪ `Blocked` (R-RS-1) |
| `update_deps` | Replace a node's dependency list — allowed only on `Pending` ∪ `Ready` ∪ `Blocked` (R-RS-1) |
| `add_before` | Insert a new node, rewire an existing node to depend on the new one |
| `add_after` | Insert a new node, take over the existing node's dependents |
| `invalidate_node` | Force a `Succeeded` node back to `Pending` (T23); cascades downstream (re-runs every dependent) |

#### 3.30.6.2 Invariants (R-RS-*)

| # | Rule |
|---|---|
| R-RS-1 | Any reshape that mutates a node MUST find the node in a non-terminal state OR in `Failed` / `ForceFailed` (whose retry path is itself a reshape-like reset). `Scheduled` and `Running` nodes are immutable to reshape — the operation is rejected with `NodeNotMutable { state, allowed }`. |
...
| R-RS-3 | Cycle detection runs on the post-reshape graph using DFS-with-three-coloring; rejection emits `ReshapeRejected { cause: "cycle", path: [nodeId, …] }`. |
| R-RS-4 | The Snapshot Validation node (`producerId == "__snapshot-validation__"`) is auto-managed: its dependency set is **always** `leaves(graph) − {__snapshot-validation__}`. Any reshape that changes the leaf set re-syncs SV's deps as part of the same atomic apply (no separate SV reshape). |
| R-RS-5 | SV node itself cannot be removed, depended-on, or have its deps explicitly updated. Operator attempts return `OperationForbidden { node: "__snapshot-validation__" }`. |
| R-RS-6 | `invalidate_node(target)` cascades: every node reachable from `target` via the dependency graph is also invalidated (back to `Pending`). The cascade is computed before apply; the user sees `ReshapeImpact { invalidated: [nodeId, …] }` in the result. |
| R-RS-7 | Reshape never preempts a Running node (FAIR-4). If an operation requires mutating a Running node, it is rejected; the operator must wait for the node to terminate (or force-fail it via T15) and then reshape. |
| R-RS-8 | Reshape acquires the plan's writer lease (§3.30.7) for the duration of the validate-and-apply, not just the apply. This ensures that a sweep cannot dispatch a node that reshape just removed. |
| R-RS-9 | Reshape emits `NodeAdded` / `NodeRemoved` / `NodeReshaped` / `NodeInvalidated` events (one per affected node) followed by a single `ReshapeApplied { txnId, ops, impact }` summary event. The summary is what triggers the next scheduler sweep (R-RD-5). |
| R-RS-10 | Adding a dependency to a node currently in `Ready` reverts it to `Pending` (T6). The next sweep re-evaluates readiness — likely transitioning back to `Ready` immediately if the new dep is `Succeeded`, or to `Blocked` if it isn't. |
| R-RS-11 | Reshape cannot violate the §3.16 DAG limits (max nodes, max depth, max fan-out). Hitting a limit aborts the reshape with `ReshapeRejected { cause: "limit-exceeded", limit, current }`. |
...
Worktree-level leases (per-node in PH-1/PH-8) are a separate concern documented in §3.3 — they protect against concurrent mutation of a single worktree. The plan-store lease here is per-plan and protects the persistence tree itself.
...
| `Postcheck Auto-fix` | Postcheck failure + node has `autoHeal.postcheckRetryWithAgent = true` | PH-6 only | 1 agent retry, agent given postcheck output |
...
| # | Rule |
|---|---|
| AH-1 | Auto-heal retries occur **within the same attempt** (T13 Running→Running). They do NOT increment `attempt`. They DO emit `NodePhaseRetry { phase, retryNumber, trigger }`. |
...
| AH-4 | Operator setting `autoHeal.Enabled = false` on a node disables all auto-heal — every failure is immediately T12. Useful for "expects no changes" validation nodes where we want the failure surfaced fast. |
...
| Q-3 | If `UsedUnits ≥ BudgetUnits` (`QuotaExceededThreshold`), agent dispatch is **blocked entirely** until `WindowResetAt` or operator override. The scheduler emits `QuotaExceeded` and parks Ready agent nodes. They re-become eligible automatically at reset. |
| Q-4 | Quota-blocking does NOT mark nodes Failed — they stay Ready, just not dispatched. Operators see them in the panel as "waiting on quota". |
| Q-5 | The user can set per-plan or per-node `MaxPremiumRequests` budgets that the scheduler also enforces (a plan refusing to exceed N requests will park its agent nodes when N is reached). |
...
Cancel must propagate from user → plan → node → phase → process → agent CLI cleanly with bounded latency.

#### 3.30.10.1 Cancel cascade (CC-*)

| # | Source | Effect | Latency budget |
|---|---|---|---|
| CC-1 | `CancelAsync(planId)` RPC | Plan moves to `Canceling`; scheduler stops dispatching this plan; for every Running node, issue node-level cancel (CC-2) | < 100 ms (RPC return) |
| CC-2 | Node-level cancel (engine internal) | Cancel the phase's `CancellationTokenSource`; phase pipeline releases worktree lease; node moves to `Canceled` (T14) | < 5 s (phase teardown) |
...
| CC-6 | Plan terminal | After all nodes are terminal (Canceled or pre-existing terminal), plan moves to `Canceled`; emit `PlanCanceled` | (sum of CC-1..5) |

Total worst-case from `CancelAsync` returning to `PlanCanceled` event: **≤ 20 s** for a plan with one running agent. Multi-node plans cancel in parallel — same total bound.
...
| CC-INV-2 | Cancel does NOT roll back committed phases. A node whose Commit phase succeeded but whose MergeRi was canceled remains in the per-attempt branch — operator can reach in via git tools. |
| CC-INV-3 | Cancel propagates downstream via blocked-by-canceled (R-RD-2) — `Pending` nodes whose ancestor canceled become `Blocked`, then `Canceled` when the sweep visits them with the plan in `Canceling`. |
| CC-INV-4 | A node cannot cancel itself from within its own phase work — cancellation is always external (operator or plan-cascade). |
...
Every node failure produces a structured artifact suitable for both human inspection (panel) and machine consumption (auto-heal classifier, telemetry).
...
| # | Rule |
|---|---|
| FC-RT-1 | Failure artifacts for a node's **current attempt** are always retained until the node terminates Succeeded or the plan is deleted. |
...
| FC-RT-4 | `GetNodeFailureContextAsync` returns the artifact set for the requested attempt (defaulting to current). The result is descriptor-only (paths + byte ranges); UX layer reads the bytes (E1 efficiency rule). |
...
| RC-5 | For every node in `Scheduled` ∪ `Running`, evaluate the **orphan check**: |
|  | — If `state.json` records a `runningPid` and that pid (a) exists and (b) was started at the recorded `startedAt` and (c) the worktree path matches: declare the node **resumable** — re-attach the `IProcessLifecycle` handle (re-open log file at the persisted byte offset, re-subscribe to its output bus) and continue execution. |
|  | — Otherwise the node is **abandoned**: mark its current phase failed with `OrphanedByCrash`, transition Running→Failed (T12). User can retry. The worktree is left intact for forensic inspection. |
...
| RC-INV-2 | Recovery never destroys data — it only marks abandoned nodes failed and truncates torn `events.log` tails (with explicit logging). |
| RC-INV-3 | Recovery time is bounded: O(plans × pending-journal-entries) for the journal replay, plus O(scheduled-or-running-nodes) for orphan checks. Per-plan recovery is well under 1 s for typical sizes. |
...
| CP-3 | When the dependency plan reaches `Succeeded`, the dependent plan auto-resumes (R-DP-8 stops excluding it; the next sweep begins dispatching its Ready nodes). |
...
| `aio.nodes.scheduled` | Counter | nodes | `runner_id` |
| `aio.nodes.completed` | Counter | nodes | `outcome`, `runner_id`, `phase_failed?` |
| `aio.phase.duration` | Histogram | ms | `phase`, `runner_id`, `outcome` |
| `aio.scheduler.sweep.duration` | Histogram | ms | `trigger` |
| `aio.scheduler.dispatched_per_sweep` | Histogram | nodes | (none) |
...
| `aio.node.id` | Every node-scoped span | `01HZ…` |
| `aio.node.producer_id` | Every node-scoped span | `build-typescript` |
...
public enum ErrorCode
{
    ...
    // Node lifecycle (2xx)
    NodeNotFound = 200, NodeNotMutable = 201, NodeAlreadyTerminal = 202, NodeStateTransitionForbidden = 203,
...
```jsonc
{
  "AiOrchestrator": {
    ...
    "Scheduler":  { "MaxConcurrentNodes": 8, "SweepIntervalMs": 250 },
...
```jsonc
{
  ...
  "uptimeSeconds": 1234,
  "pid": 5678,
  "leasedPlans": [{"planId":"...","name":"...","leaseEpoch":3,"runningNodes":2}],
...
|---|---|---|---|---|
| 1 | `models-state-machine` | (none) | medium | `NodeState` enum, `INodeStateMachine`, transition table tests, analyzer `OE0042` |
...
|---|---|---|
| `OE0042` | §3.30.1.4 | Node-state transitions match the §3.30.1.2 table; SV node guarded |
...
1. The full `IAiOrchestrator` contract as JSON-RPC methods (`plan.scaffold`, `plan.finalize`, `node.retry`, …).
...
ai-orchestrator plan logs <planId> <nodeId> [--phase work] [--attempt 2]
ai-orchestrator plan watch <planId>           # streams events to stdout
ai-orchestrator plan reshape <planId> --add-job …
ai-orchestrator node retry <planId> <nodeId> [--clear-worktree]
...
The `compatShim` keeps the UI code unchanged: the Plan Sidebar, Plan Detail Panel, Node Detail Panel, Status Bar, and BulkPlanActions all keep their current consumption pattern (event emitter + method calls), but those calls are now serialized through whichever transport `transportSelector` chose.
...
2. **MCP server lifecycle** (`ai-orchestrator-mcp`): NOT spawned by the extension. The extension instead **registers** `ai-orchestrator-mcp` in the user's `mcp.json` at activation (configurable / opt-in), so Copilot Chat (or any other MCP host) is the one that spawns it on demand. This decouples MCP from the extension lifecycle entirely — the extension can be uninstalled and Copilot Chat continues to drive the daemon over MCP.
   ...
   **The pub/sub flow itself is the same regardless of mode** (this is the part the question calls out): once `ai-orchestrator-mcp` has authenticated to the daemon, it opens a single long-lived `IEventBus.SubscribeAsync` stream per MCP client session. Each event the bus emits is forwarded to the MCP host as an MCP `notifications/ai-orchestrator.event` message tagged with the originating `planId` / `nodeId` / `runId`. The MCP host (Copilot Chat) routes those notifications back into the agent's context per its own UX conventions. Key invariants:
...
A prebuilt N-API native module that exposes the daemon's `IAiOrchestrator` and `IEventBus` to Node.js with **zero JSON-RPC overhead** when running in-process. Built once per RID using `Node.NET` / `NetCore.Node` interop or hand-rolled N-API + a managed bootstrapper, and shipped on npm as `@ai-orchestrator/native`.
...
const events: EventStream = orch.subscribe({ planId, categories: ['Node', 'Agent'] });
...
- We load into a **named `AssemblyLoadContext`** (`"AiOrchestrator"`) so our types are identity-isolated from any other `.NET-in-Node` consumer (C# DevKit, future Razor extensions).
...
.NET's `ThreadPool` and Node's libuv pool are independent OS-thread pools competing for the same cores. Our defaults:
...
sequenceDiagram
    ...
    Core-->>Daemon: NodeTransition events
    Daemon-->>Pipe: events
    Pipe-->>Ext: events
...
sequenceDiagram
    ...
    ChatMcp->>Bus: Subscribe filter planId X categories Node Plan
    CliWatch->>Bus: Subscribe filter planId X categories all
    Core-->>Bus: NodeTransition pending to ready
...
- **Plan Detail Panel** — same Mermaid graph, same node list.
- **Node Detail Panel** — same logs/attempts/metrics.
- **Status Bar** — same progress indicator.
- **Bulk Plan Actions** — same multi-select operations.

The only difference is plumbing: the events arriving from the daemon are translated by `compatShim.ts` into the same shape today's UI expects (`'planCreated'`, `'nodeTransition'`, `'nodeStarted'`, …).
...
- **Exit codes**: 0 success, 2 plan failed, 3 node failed, 4 connection error, 5 auth error.
...
| `<node8>` | First 8 hex chars of the node UUID (collision-safe within a plan) | 8 |

**Worktree path budget on Windows:** `C:\src\<repo>\.wt\<plan8>\<node8>\<attempt>\` consumes only ~30 bytes of fixed overhead beyond the repo root — e.g. `.wt\9f3c8a4b\6e1d77c0\1\` is 23 bytes. Compare to today's `.worktrees/<full-uuid>/<full-uuid>/` which alone burns 88 bytes.
...
│   │   │   └── n/                     ← node specs ("n" not "nodes")
│   │   │       ├── 6e1d77c0/          ← node id, first 8 hex
...
└── .wt/                               ← .gitignored; root-level for shortest possible path to compiled artifacts (see §8.3)
    └── 9f3c8a4b/6e1d77c0/<attempt>/   ← <plan8>/<node8>/<attempt> — ~23 bytes overhead
```
...
| `.aio/p/<plan8>/n/<node8>/INSTRUCTIONS.md` | **yes** | Agent instructions | Lets reviewers see exactly what each AI was told to do — **critical for security/audit review of agent-generated PRs.** |
| `.aio/.ps/state/*.json` | **no** | Live execution state (cursors, attempts, current node statuses) | Per-clone; not portable; recoverable via `Recover` from the immutable definition + git commits. |
...
| `.wt/**` | **no** (gitignored) | Active worktree checkouts | Created/destroyed per node execution. |

**Recovery contract:** if `.aio/.ps/` is deleted, `ai-orchestrator plan recover <planId>` rebuilds runtime state from `.aio/p/<plan8>/` plus the git commit graph (each node's commit references the planId/nodeId in its trailer). This is the same `recover_copilot_plan` semantic that exists today, formalized.
...
Every node executes inside its own git worktree at `<repoRoot>/.wt/<plan8>/<node8>/<attempt>/`. The worktree is **never reused across attempts unless explicitly preserved**, which keeps each attempt's blast radius bounded.
...
- **`MAX_PATH` = 260 chars** — still hit by some legacy tools (older MSBuild tasks, some Node CLIs that don't use `\\?\` long-path prefix even when the OS allows it).
...
Our fixed overhead from repo root must stay tiny. The `.wt/<plan8>/<node8>/<attempt>/` shape costs **23 bytes** (e.g. `.wt\9f3c8a4b\6e1d77c0\1\`). That leaves a developer who clones to `C:\src\<repo>\` (~14 bytes) with **~163 bytes** of room for source-tree paths before hitting the 200-char `node_modules` ceiling.

#### Worktree creation pipeline

The `setup` phase (one of the eight phases per node — see §6.1) runs this sequence:

```mermaid
flowchart LR
    A[Node ready] --> B[Compute base commit]
    B --> C[git worktree add --detach -b aio/plan8/node8/attempt]
...
1. **Base commit selection.** Root nodes branch from the plan's `baseBranch` (default `main`). Dependent nodes branch from the merge commit produced by the deepest successful ancestor (this is what gives the DAG its FI semantics — see §3.3).

2. **`git worktree add`** at `<repoRoot>/.wt/<plan8>/<node8>/<attempt>/` with a deterministic per-attempt branch name: `aio/<plan8>/<node8>/<attempt>`. The branch is **detached-style** (no upstream) so accidental pushes don't pollute the remote. The branch is deleted at cleanup unless preserved (see retention). The `aio/` ref namespace prefix (instead of the older `plan/`) avoids collision with user-authored `plan/*` branches and is short enough to fit in `git log --oneline` output without truncation.
...
|---|---|
| Node `succeeded` and `cleanUpSuccessfulWork: true` | Worktree removed via `git worktree remove --force`; branch `aio/<plan8>/<node8>/<attempt>` deleted; commit kept (already merged into snapshot). |
| Node `failed` (terminal) | Worktree **preserved** at `.wt/<plan8>/<node8>/<attempt>/` for debugging; branch preserved as `aio/failed/<plan8>/<node8>/<attempt>`; surfaced in `get_node_failure_context`. |
| Node `canceled` | Worktree preserved if it had any uncommitted changes; otherwise removed. |
...
| **Reachability** | Every job must be reachable from at least one root, and every job must reach the `__snapshot-validation__` node. |
| **ProducerId uniqueness** | Within a plan; case-sensitive; reserved prefix `__` only for orchestrator-managed nodes. |
...
{ "type": "process", "executable": "node", "args": ["build.js"], "env": { … } }
...
`ai-orchestratord` and `ai-orchestrator-mcp` ship with the extension VSIX. We deliberately do **not** support out-of-band native binary updates from a different channel — that would create version-skew between the bundled `AiOrchestrator.Bindings.Node` (which is `dlopen`'d into the extension's Node process) and the daemon. All three move together as one VSIX.
...
| `AiOrchestrator.Bindings.Node` (interop boundary, Phase 0/4) | **75%** | **70%** (much of the surface is JS-side, covered by Node tests) |

...
public sealed class RuleIdCoverageGateTests
{
    ...
    public async Task RULE_COVERAGE_REPORT_LogsExtraRules()
    {
        ...
        Assert.NotNull(report.ExtraTestRuleIds);
        Assert.Equal(report.UncoveredRuleIds.IsEmpty, report.Ok);
    }
    ...
    public void DesignDocRuleExtractor_OnlyMatchesIdsWithinSection()
    {
        ...
        Assert.Contains("FOO-BAR-1", ids);
        Assert.Contains("BAZ-2", ids);
        Assert.DoesNotContain("DROP-1", ids);
        Assert.DoesNotContain("QUX-9", ids);
    }
    ...
    public async Task ContractTestScanner_ReturnsLiteralIds()
    {
        ...
        Assert.NotEmpty(ids);
        Assert.Contains("ACCEPT-COVERAGE", ids);
        Assert.Contains("ACCEPT-COVERAGE-EXTRA", ids);
    }
}

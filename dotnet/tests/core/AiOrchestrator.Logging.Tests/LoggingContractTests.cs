...
public sealed class LoggingContractTests
{
    ...
    public void LOG_2_IncludesRequiredFields()
    {
        ...
        Assert.True(json.TryGetProperty("level", out _));
        Assert.True(json.TryGetProperty("category", out var cat));
        Assert.True(json.TryGetProperty("message", out var msg));
        Assert.Equal("MyCategory", cat.GetString());
        Assert.Equal("Hello World", msg.GetString());
    }
    ...
    public void LOG_3_AppliesRedactor_BeforeWrite()
    {
        ...
        var json = JsonDocument.Parse(writer.ToString().Trim()).RootElement;
        Assert.StartsWith("[REDACTED]", json.GetProperty("message").GetString());
    }
    ...
    public void LOG_8_FormatterException_EmitsFallbackAndRethrows()
    {
        ...
        var output = writer.ToString();
        Assert.Contains("LoggerFormatterFailure", output);
    }
    ...
    public void LOG_5_AmbientContext_RestoredOnDispose()
    {
        ...
        {
            Assert.Equal("inner", AmbientContext.Get<string>(key));
        }

        // Assert — outer value is restored after inner scope is disposed
        Assert.Equal("outer", AmbientContext.Get<string>(key));
    }
    ...
    public async Task LOG_6_AmbientContext_DoesNotLeakAcrossTests()
    {
        ...
        Assert.Null(AmbientContext.Get<string>(leakKey));
    }

...
public sealed class TieredEventLogContractTests : IDisposable
{
    ...
    public void T2_LOG_1_FrameLayoutMatchesSpec()
    {
        ...
        Assert.Equal((uint)payload.Length, BinaryPrimitives.ReadUInt32LittleEndian(dest));
        Assert.Equal(42L, BinaryPrimitives.ReadInt64LittleEndian(dest.AsSpan(4)));
        ...
        Assert.Equal(expectedCrc, trailerCrc);
        ...
        var parsed = RecordFramer.TryUnframe(
            ...
            out var err);
        Assert.True(parsed);
        Assert.Equal(FrameError.None, err);
        Assert.Equal(42, rec.RecordSeq);
        Assert.Equal(payload, rec.Payload.ToArray());
    }
    ...
    public void T2_LOG_2_CrcMismatchIsFlagged()
    {
        ...
        var ok = RecordFramer.TryUnframe(
            ...
            out var err);
        Assert.False(ok);
        Assert.Equal(FrameError.CrcMismatch, err);
    }
    ...
    public async Task T2_READ_1_ReplayResumesFromLastValidRecord()
    {
        ...
        Assert.Equal(new long[] { 1, 2, 3, 4 }, read.Select(e => e.RecordSeq).ToArray());
    }
    ...
    public void T2_READ_11_ReassemblyAbandonedAfterBudget()
    {
        ...
        Assert.True(buffer.TryAppend(new byte[60]));

        // Exceed the cap: abandoned.
        Assert.False(buffer.TryAppend(new byte[10]));

        // After the abandonment signal the timeout has not elapsed (no time has passed).
        Assert.False(buffer.IsTimedOut());
    }
    ...
    public void T2_READ_11_RecordSeqGapDetected()
    {
        ...
        var ok = RecordFramer.TryUnframe(
            ...
            out var err);
        Assert.False(ok);
        Assert.Equal(FrameError.RecordSeqRegression, err);
    }
    ...
    public async Task SUB_3_ReplayToLive_NoDuplicates_NoGaps()
    {
        ...
        {
            Assert.Equal(1, ordered[i] - ordered[i - 1]);
        }
    }
    ...
    public async Task DISK_PLAN_1_PerPlanCapEnforced()
    {
        ...
        Assert.True(hit, "the per-plan cap must trip eventually");
    }
    ...
    public async Task DISK_PLAN_2_ExceededExceptionIncludesPlanId()
    {
        ...
        Assert.Equal(planId, caught!.PlanId);
        Assert.Equal(64, caught.Cap);
        Assert.True(caught.Requested > 0);
        Assert.Contains(planId.ToString(), caught.Message);
    }
    ...
    public async Task T3_ARCHIVE_1_ColdSegmentsCompressedAfterAge()
    {
        ...
        Assert.False(File.Exists(seg));
        Assert.True(File.Exists(seg + ".gz"));
...
internal sealed class InertFileSystem : IFileSystem
{
    ...
    public ValueTask<Stream> OpenWriteAsync(AbsolutePath path, CancellationToken ct)
        => new(new FileStream(path.Value, FileMode.Create, FileAccess.Write));

    public ValueTask<Stream> OpenAppendAsync(AbsolutePath path, CancellationToken ct)
        => new(new FileStream(path.Value, FileMode.Append, FileAccess.Write));
}

...
public sealed class CredentialBrokerContractTests
{
    ...
    public async Task CRED_ACL_1_DisallowedHostRejected()
    {
        ...
        await Assert.ThrowsAsync<CredentialHostNotAllowedException>(act);
...
Assert.StartsWith(@"\\.\pipe\ai-orchestrator", path);
        ...
        {
Assert.StartsWith("/run/ai-orchestrator/cred.", path);
Assert.False(path.StartsWith("@"), "abstract-namespace sockets are forbidden (CRED-IPC-1)");
Assert.False(path.StartsWith("\0"), "abstract-namespace sockets are forbidden (CRED-IPC-1)");
        }
    }
...
Assert.Equal("/@abstract", new CredentialIpc(bad, spawner).SocketPath);
        }
    }
...
Assert.Equal("uid_mismatch", evt.Reason);
...
Assert.Equal("super-sekret-42", cred.Password.Reveal());
...
Assert.Contains("get", verbs); Assert.Contains("store", verbs);
...
Assert.Contains(audit.Records, r => r.EventType == "credential.store");
    ...
    public async Task CRED_VERB_1_FillEraseOnAuthFailure()
    {
        ...
        var verbs = spawner.SpawnedSpecs.Select(s => s.Arguments[0]).ToArray();
Assert.Contains("get", verbs); Assert.Contains("erase", verbs);
Assert.Contains(audit.Records, r => r.EventType == "credential.erase");
    ...
    public async Task CRED_TIMEOUT_1_GcmKilledAfterTimeout()
    {
        ...
        Assert.True(ex is GcmInvocationException || ex is OperationCanceledException);
    }
    ...
    public async Task CRED_INVAL_1_ExponentialBackoffEngagesAfter5Failures()
    {
        ...
        var act = async () => await broker.GetAsync(url, TestPrincipals.Alice(), default);
        await Assert.ThrowsAsync<CredentialBackoffActiveException>(act);
Assert.NotEmpty(bus.Published.OfType<CredentialBackoffEngaged>());
    }
...
Assert.DoesNotContain(secret, ps.ToString());
...
Assert.DoesNotContain(secret, cred.ToString());
    ...
    public async Task CRED_AUDITED_OnEveryOperation()
    {
        ...
        var eventTypes = audit.Records.Select(r => r.EventType).ToList();
        Assert.Contains("credential.get", eventTypes);
        Assert.Contains("credential.store", eventTypes);
        Assert.Contains("credential.erase", eventTypes);

        foreach (var r in audit.Records)
        {
            Assert.DoesNotContain("should-not-leak", r.ContentJson);
Assert.DoesNotContain("super-sekret-42", r.ContentJson);
        }
    }
}

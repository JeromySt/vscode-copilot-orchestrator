...
Assert.Equal("mount", spawner.SpawnedSpecs[0].Executable);
Assert.Contains("--bind", spawner.SpawnedSpecs[0].Arguments);
    }
...
Assert.NotEmpty(spawner.SpawnedSpecs);
Assert.Equal("cmd.exe", spawner.SpawnedSpecs[0].Executable);
Assert.Contains("/J", spawner.SpawnedSpecs[0].Arguments);
    }
...
Assert.False(probe.IsImmutabilitySupported(result));
        var evt = probe.BuildEvent(new AbsolutePath(System.IO.Path.GetTempPath()), result);
Assert.False(string.IsNullOrEmpty(evt.Mechanism));
Assert.False(string.IsNullOrEmpty(evt.Reason));
    }
...
Assert.NotNull(mgr.Previous);
Assert.Equal(original.Value, mgr.Previous!.Value);
    }
...
Assert.Equal(3, rpc.PeerCredChecksPerformed);
Assert.Equal(3, daemon.PeerCredChecksPerformed);
...
Assert.NotNull(approval);
        ...
        var act = async () => await daemon.HandleCheckInAsync(req, CancellationToken.None);
        var ex = await Assert.ThrowsAsync<HookApprovalDeniedException>(act);
Assert.Equal("daemon shutting down", ex.Reason);
    }
}

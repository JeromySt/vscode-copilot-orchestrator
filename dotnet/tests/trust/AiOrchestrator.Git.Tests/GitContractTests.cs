...
Assert.False(cb(tp));
    ...
    public async Task LG2_CANCEL_2_CancellationCompletesUnder200ms()
    {
        ...
        sw.Stop();
Assert.True(sw.Elapsed < TimeSpan.FromMilliseconds((2 * opts.ProgressTickInterval.TotalMilliseconds) + 200));
    }
...
Assert.IsType<MergeConflictException>(conflict);
Assert.NotEmpty(((MergeConflictException)conflict).ConflictingPaths);
    }
...
Assert.IsType<RemoteRejectedException>(mapped);
        var rr = (RemoteRejectedException)mapped;
Assert.Contains("example.com", rr.RemoteUrl);
    }
...
Assert.IsType<AuthFailureException>(mapped);
Assert.Equal("example.com", ((AuthFailureException)mapped).RepoUrl.Host);
    }
    ...
    public async Task LG2_BRK_4_RefRaceMappedToTypedException()
    {
        ...
        await Assert.ThrowsAsync<RefUpdateRaceException>(act);
    }
...
Assert.Contains("--lock", spawner.Calls[0].Arguments);
Assert.Contains("remove", spawner.Calls[1].Arguments);
Assert.Contains("--force", spawner.Calls[1].Arguments);
    }
...
Assert.DoesNotContain("hunter2", mapped.Message);
Assert.DoesNotContain("alice:hunter2", mapped.Message);
Assert.Contains("***:***@", mapped.Message);
    }
...
Assert.Equal(fixedTime, info.AuthorDateUtc);
Assert.Equal(fixedTime, info.CommitterDateUtc);
    }
}

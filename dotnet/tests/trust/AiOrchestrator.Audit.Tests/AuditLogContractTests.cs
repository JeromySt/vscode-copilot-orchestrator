...
Assert.True(result.Ok, $"chain should remain valid across rotation but failed: {result.Reason} {result.Detail}");
Assert.Equal(2, segments.Count);
    ...
    public async Task KEY_ROT_1_MissingCrossSignatureRejected()
    {
        ...
        var bogusNewSig = new byte[72]; // all zeros — will not verify
...
Assert.Equal(ChainBreakReason.AuditChainKeyTransitionMissingCrossSignature, result.Reason);
    }
...
Assert.False(result.Ok);
Assert.Equal(ChainBreakReason.AuditChainBrokenAtInstallAnchor, result.Reason);
    }
...
Assert.False(result.Ok, "an unsigned manifest cannot extend trust");
        Assert.Contains(result.Reason!.Value, new[] {
    ...
    // Roslyn-style: scan source files for EcdsaSigner.Sign call sites and
    ...
    public void TRUST_ROOT_4_DaemonNeverSignsWithBuildKey()
    {
        ...
        foreach (var file in allCsFiles)
        {
            ...
            var allowed = rel.Contains("AiOrchestrator.Audit/AuditLog.cs", StringComparison.Ordinal)
                ...
                || rel.Contains("AiOrchestrator.Audit/Crypto/EcdsaSigner.cs", StringComparison.Ordinal)
...
Assert.False(resultStrict.Ok);
Assert.Equal(ChainBreakReason.AuditChainTransparencyLogMismatch, resultStrict.Reason);
...
Assert.Equal(2, segments.Count);
        ...
        var result = verifier.Verify(segments, VerifyMode.Standard);
Assert.True(result.Ok, $"chain should survive update, got {result.Reason}: {result.Detail}");
    }
...
Assert.False(verification.Ok);
        Assert.Contains(verification.Reason!.Value, new[] {
...
Assert.True(File.Exists(stray));
        ...
        await using var log2 = this.Build().Log;
Assert.False(File.Exists(stray), "stray .tmp must be cleaned on startup");
...
Assert.True(sizeRolledFiles.Count >= 3, "size threshold must produce 1 segment per append");
...
Assert.True(timeRolledFiles.Count >= 2, "time threshold must produce a fresh segment after the window passes");
    }
...
Assert.False(result.Ok);
Assert.Equal(ChainBreakReason.AuditChainSegmentSeqRegression, result.Reason);
    }

...
public sealed class PerUserConcurrencyLimiterTests
{
    ...
    public async Task CONC_USER_FifoOrderingPreserved()
    {
        ...
        Assert.Equal(jobIds[1..], admissionOrder);
    }
    ...
    public async Task CONC_USER_QueuedEventEmittedExactlyOnce()
    {
        ...
        Assert.Equal(1, bus.Of<ConcurrencyUserQueued>().Count);
        Assert.Equal(job2, bus.Of<ConcurrencyUserQueued>()[0].JobId);
        Assert.Equal(0, bus.Of<ConcurrencyUserQueued>()[0].QueuePosition);
    ...
    public async Task CONC_USER_AdmittedEventIncludesWaitTime()
    {
        ...
        Assert.Contains(admittedEvents, e => e.JobId == job2);

        var admittedEvent = admittedEvents.Single(e => e.JobId == job2);
        Assert.True(admittedEvent.WaitTime > TimeSpan.Zero, "WaitTime must reflect actual queue wait");
        Assert.InRange(admittedEvent.WaitTime, delay - TimeSpan.FromMilliseconds(50), delay + TimeSpan.FromMilliseconds(50));
        Assert.Equal(clock.UtcNow, admittedEvent.At);
    }
    ...
    public async Task CONC_USER_DisposeReleasesSlot()
    {
        ...
        Assert.Equal(job2, second.JobId);
        Assert.NotEqual(default, second.AdmittedAt);
    ...
    public async Task CONC_USER_CancellationRemovesWaiter()
    {
        ...
        Assert.Equal(jobC, admittedC.JobId);
    }
    ...
    public async Task CONC_USER_PerUserIsolation()
    {
        ...
        Assert.Equal("user-B", bAdmission.Principal.PrincipalId);

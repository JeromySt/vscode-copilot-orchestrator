...
public sealed class PluginsCoverageGap2Tests : IDisposable
{
    ...
    private sealed class PassthroughFs : IFileSystem
    {
        ...
        public ValueTask CreateDirectoryAsync(AbsolutePath path, CancellationToken ct) { Directory.CreateDirectory(path.Value); return ValueTask.CompletedTask; }
        ...
        public async ValueTask<byte[]> ReadAllBytesAsync(AbsolutePath path, CancellationToken ct) => await File.ReadAllBytesAsync(path.Value, ct);

        public async ValueTask WriteAllBytesAsync(AbsolutePath path, byte[] contents, CancellationToken ct) => await File.WriteAllBytesAsync(path.Value, contents, ct);

        public ValueTask CopyAsync(AbsolutePath source, AbsolutePath destination, bool overwrite, CancellationToken ct) { File.Copy(source.Value, destination.Value, overwrite); return default; }
        ...
        public ValueTask<Stream> OpenWriteAsync(AbsolutePath path, CancellationToken ct) => new(new FileStream(path.Value, FileMode.Create, FileAccess.Write));

        public ValueTask<Stream> OpenAppendAsync(AbsolutePath path, CancellationToken ct) => new(new FileStream(path.Value, FileMode.Append, FileAccess.Write));
    }

...
public sealed class PluginLoaderContractTests : IDisposable
{
    ...
    public async Task TRUST_ACL_1_BroadPermsRejected()
    {
        ...
        Assert.Contains("TRUST-ACL-1", ex.Message);

        Assert.Contains(events, e => e is PluginTrustFileInvalidPerms);
    }
    ...
    public async Task TRUST_ACL_2_BadSignatureRejected()
    {
        ...
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(act);
        Assert.Contains("Ed25519 signature is invalid", ex.Message);
    }
    ...
    public async Task TRUST_ACL_3_HashMismatchRejected()
    {
        ...
        Assert.Single(events.OfType<PluginRejected>(), e => e.Reason.Contains("SHA-256", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task PLUGIN_MANIFEST_MissingRejected()
    {
        ...
        Assert.Empty(results);
        Assert.Single(events.OfType<PluginRejected>(), e => e.Reason.Contains("manifest", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public async Task PLUGIN_ALC_IsolatesAssemblies()
    {
        ...
        Assert.NotSame(type2.Assembly, type1.Assembly);
    ...
    public async Task PLUGIN_CAPABILITY_DeclaredCapabilitiesEnforced()
    {
        ...
        Assert.False(checker.IsAllowed(plugin, PluginCapability.NetworkAccess));

        var capEx = await Assert.ThrowsAsync<PluginCapabilityDeniedException>(
            () => checker.EnforceAtCallSiteAsync("cap-plugin", PluginCapability.NetworkAccess).AsTask());
        Assert.Contains("NetworkAccess", capEx.Message);
    }
    ...
    public async Task PLUGIN_AUDIT_LoadAndRejectAreAudited()
    {
        ...
        Assert.Equal(1, results.Length);
        Assert.Equal("valid-plugin", results[0].PluginId);

        // Audit log should have PluginLoaded + PluginRejected events
        Assert.Contains(audit.Records, r => r.EventType == "PluginLoaded");
        Assert.Contains(audit.Records, r => r.EventType == "PluginRejected");

        // Event bus should have corresponding events
        Assert.Contains(events, e => e is PluginLoaded);
        Assert.Contains(events, e => e is PluginRejected);
...
internal sealed class PassthroughFileSystem : IFileSystem
{
    ...
    public async ValueTask<byte[]> ReadAllBytesAsync(AbsolutePath path, CancellationToken ct)
        => await File.ReadAllBytesAsync(path.Value, ct);

    public async ValueTask WriteAllBytesAsync(AbsolutePath path, byte[] contents, CancellationToken ct)
        => await File.WriteAllBytesAsync(path.Value, contents, ct);

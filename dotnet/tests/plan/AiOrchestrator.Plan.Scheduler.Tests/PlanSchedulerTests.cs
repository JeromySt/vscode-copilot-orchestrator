...
public sealed class PlanSchedulerTests
{
    ...
    public async Task SCHED_PAUSE_NoNewAdmissions_InFlightCompletes()
    {
        ...
        try
        {
            ...
            Assert.Equal(0, phaseExec.ExecuteCallCount);
        }
        finally
        {
    ...
    public async Task SCHED_CANCEL_AllPendingFlippedToCanceled()
    {
        ...
        try
        {
            ...
            var jobMutEntry = Assert.Single(mutations, m => m.Mut is JobStatusUpdated);
            var jobMut = jobMutEntry.Mut as JobStatusUpdated;
            Assert.Equal(JobStatus.Canceled, jobMut!.NewStatus);
            Assert.Equal(pendingJobId.ToString(), jobMut.JobIdValue);

            var planMutEntry = Assert.Single(mutations, m => m.Mut is PlanStatusUpdated);
            var planMut = planMutEntry.Mut as PlanStatusUpdated;
            Assert.Equal(PlanStatus.Canceled, planMut!.NewStatus);
        }
        finally
        {
            await scheduler.StopAsync(CancellationToken.None);
    ...
    public async Task SCHED_ADMIT_FailureRequeues()
    {
        ...
        try
        {
            ...
            Assert.True(calledTwice, "after admission failure the job must be re-queued and retried (INV-8)");
        }
        finally
        {
            await scheduler.StopAsync(CancellationToken.None);
    ...
    private sealed class StubPlanStore : IPlanStore
    {
        ...
        public async Task WaitForWatchStartedAsync(TimeSpan timeout)
        {
            ...
            Assert.True(acquired, "WatchAsync must be called within the timeout");
        }

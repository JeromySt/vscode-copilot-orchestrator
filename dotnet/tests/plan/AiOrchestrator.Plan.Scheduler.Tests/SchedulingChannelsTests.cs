...
public sealed class SchedulingChannelsTests
{
    ...
    public async Task CONC_CHAN_1_BoundedAtCapacity()
    {
        ...
        Assert.False(secondWrite.IsCompleted, "channel is at capacity; second write must block (CONC-CHAN-1)");

        // Drain the channel; second write should now complete.
        var _ = await channels.ReadyChannel.Reader.ReadAsync(cts.Token);
        await secondWrite;
        Assert.True(secondWrite.IsCompletedSuccessfully, "second write completes after the channel is drained");
        channels.Complete();
    }
    ...
    public void CONC_CHAN_2_DedupesEventKeyInWindow()
    {
        ...
        Assert.True(channels.TryDedup(planId, jobId, EventType, 1000), "first event is never a duplicate");

        // Same key at ms=1001: within 1-second window → duplicate.
        Assert.False(channels.TryDedup(planId, jobId, EventType, 1001), "same key within dedup window must be suppressed (CONC-CHAN-2)");

        // Same key at ms=2001: outside window → not a duplicate.
        Assert.True(channels.TryDedup(planId, jobId, EventType, 2001), "same key outside dedup window must pass through");

        // Different event type: never a duplicate for the same key.
        Assert.True(channels.TryDedup(planId, jobId, "scheduled", 1001), "different eventType is a distinct key");
        channels.Complete();
    }
}

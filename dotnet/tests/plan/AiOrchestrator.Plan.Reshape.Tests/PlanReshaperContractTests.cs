...
public sealed class PlanReshaperContractTests : IAsyncLifetime
{
    ...
    public async Task RS_TXN_1_BatchAtomicValidation()
    {
        ...
        Assert.Contains(ex.Failures, f => f.FailureReason == "UnknownJob");

        // RS-TXN-1 atomicity: nothing was persisted.
        Assert.Equal(journalBefore, (await this.CountJournalAsync(planId)));
        var loaded = await this.store.LoadAsync(planId, CancellationToken.None);
        Assert.DoesNotContain(loaded!.Jobs.Values, j => j.Title == "B");
    }
    ...
    public async Task RS_TXN_2_BatchUsesSingleIdempotencyKey()
    {
        ...
        Assert.Equal(2, result.PerOperation.Count());
        ...
        Assert.Equal(2, reshapeMutations.Select(m => m.IdemKey).Distinct().Count());
    ...
    public async Task RS_AFTER_1_RewiresSuccessors()
    {
        ...
        Assert.Equal(ids["A"].ToString(), Assert.Single(loaded.Jobs[newId.ToString()].DependsOn));
        Assert.Equal(newId.ToString(), Assert.Single(loaded.Jobs[ids["B"].ToString()].DependsOn));
        Assert.Equal(newId.ToString(), Assert.Single(loaded.Jobs[ids["C"].ToString()].DependsOn));
    }
    ...
    public async Task RS_AFTER_2_CycleDetectedBeforeApply()
    {
        ...
        var ex = await Assert.ThrowsAsync<ReshapeRejectedException>(act);
        Assert.Equal("Cycle", Assert.Single(ex.Failures).FailureReason);

        // Cycle detected BEFORE any mutation landed.
        Assert.Equal(journalBefore, (await this.CountJournalAsync(planId)));
        var loaded = await this.store.LoadAsync(planId, CancellationToken.None);
        Assert.Empty(loaded!.Jobs[ids["A"].ToString()].DependsOn);
    }
    ...
    public async Task DAG_LIM_1_MaxJobsCheckedAtReshape()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(
            ...
            CancellationToken.None);
        var ex = await Assert.ThrowsAsync<ReshapeRejectedException>(act);
        Assert.Equal("DagLimitExceeded:MaxJobs", Assert.Single(ex.Failures).FailureReason);
    }
    ...
    public async Task DAG_LIM_1_MaxParallelCheckedAtReshape()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(
            ...
            CancellationToken.None);
        var ex = await Assert.ThrowsAsync<ReshapeRejectedException>(act);
        Assert.Equal("DagLimitExceeded:MaxParallel", Assert.Single(ex.Failures).FailureReason);
    }
    ...
    public async Task RS_REMOVE_RunningJobRejected()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(
            ...
            CancellationToken.None);
        var ex = await Assert.ThrowsAsync<ReshapeRejectedException>(act);
        Assert.Equal("StatusNotRemovable", Assert.Single(ex.Failures).FailureReason);
    }
    ...
    public async Task RS_UPDATE_DEPS_RunningJobRejected()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(
            ...
            CancellationToken.None);
        var ex = await Assert.ThrowsAsync<ReshapeRejectedException>(act);
        Assert.Equal("StatusNotUpdatable", Assert.Single(ex.Failures).FailureReason);
    }
    ...
    public async Task RS_BATCH_LIMIT_Enforced()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(
            ...
            CancellationToken.None);
        var ex = await Assert.ThrowsAsync<ReshapeRejectedException>(act);
        Assert.Equal("BatchLimitExceeded", Assert.Single(ex.Failures).FailureReason);
    }
}

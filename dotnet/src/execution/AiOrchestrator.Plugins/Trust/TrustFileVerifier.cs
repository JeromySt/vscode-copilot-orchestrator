...
/// Checks file permissions (owner-only) and validates the ECDSA P-256 signature over the canonical payload.
/// </summary>
internal sealed class TrustFileVerifier
{
    ...
    /// <summary>
    /// Loads the trust file from <paramref name="trustFile"/>, verifies its ECDSA P-256 signature
    /// against <paramref name="expectedSignerPubKey"/>, and returns the deserialized
    /// <see cref="TrustFile"/> if valid.
    /// </summary>
    /// <param name="trustFile">Absolute path to the trust file.</param>
    /// <param name="expectedSignerPubKey">ECDSA P-256 public key in SubjectPublicKeyInfo DER format.</param>
    /// <param name="ct">Cancellation token.</param>
    ...
    public async ValueTask<TrustFile> LoadAndVerifyAsync(AbsolutePath trustFile, byte[] expectedSignerPubKey, CancellationToken ct)
    {
        ...
        if (!VerifyEcdsaP256(payload, tf.Ed25519Signature, expectedSignerPubKey))
        {
    ...
    private static bool VerifyEcdsaP256(byte[] message, byte[] signature, byte[] publicKey)
    {
        try
        {

...
public sealed class PlanStoreContractTests : IDisposable
{
    ...
    public async Task PLAN_STORE_RoundTrip_CreateLoad()
    {
        ...
        loaded.Jobs.Should().ContainKey("job-1");
        loaded.Jobs["job-1"].Status.Should().Be(JobStatus.Running);
    }
    ...
    public async Task RW_2_IDEM_1_RetryWithSameKeyAndContentIsNoop()
    {
        ...
        entries.Should().HaveCount(1, "retried idempotent mutation must be a no-op");
    }
    ...
    public async Task RW_2_IDEM_2_RetryWithSameKeyDifferentContentThrows()
    {
        ...
        ex.Which.Key.Should().Be(idem);
        ex.Which.StoredMutation.Should().NotBeNull();
        ex.Which.NewMutation.Should().NotBeNull();
        ((JobAdded)ex.Which.StoredMutation).Node.Id.Should().Be("j1");
        ((JobAdded)ex.Which.NewMutation).Node.Id.Should().Be("j2");
    }
    ...
    public void RW_2_IDEM_3_KeyComputedFromCanonicalContent()
    {
        ...
        k1.Should().Be(k2, "SHA-256 of identical content must produce identical keys (deterministic across calls/processes)");
        k1.Value.Should().HaveLength(64);
        k1.Value.Should().MatchRegex("^[0-9A-F]+$");
    ...
    public async Task PLAN_STORE_CheckpointAtomicOnCrash()
    {
        ...
        var loaded = await store2.LoadAsync(id, CancellationToken.None);
        loaded.Should().NotBeNull();
        loaded!.Jobs.Should().ContainKey("j");

        // Final checkpoint file is a single valid JSON document.
        File.Exists(cpPath).Should().BeTrue();
        var doc = System.Text.Json.JsonDocument.Parse(File.ReadAllText(cpPath));
        doc.RootElement.GetProperty("plan").GetProperty("name").GetString().Should().Be("p");
    }
    ...
    public async Task PLAN_STORE_WatchReplayThenLive_NoGapNoDup()
    {
        ...
        received[0].Jobs.Should().ContainKey("j1");

        // Subsequent snapshots reflect live mutations — no gap, no dup.
        received[1].Jobs.Should().ContainKeys("j1", "j2");
        received[2].Jobs["j1"].Status.Should().Be(JobStatus.Running);
    ...
    public async Task PLAN_STORE_JournalGapDetected()
    {
        ...
        await act.Should().ThrowAsync<PlanJournalCorruptedException>();
    }
    ...
    public async Task PLAN_STORE_AutoCheckpointTriggers()
    {
        ...
        upToSeq.Should().BeGreaterThanOrEqualTo(2, "auto-checkpoint should have captured at least up through the 3rd mutation (seq 2)");
    }
    ...
    public async Task PLAN_STORE_ListEnumeratesLazily()
    {
        ...
        all.Select(p => p.Name).Should().BeEquivalentTo(new[] { "p0", "p1", "p2", "p3", "p4" });
    }
...
public sealed class PlanStoreCoverageTests : IDisposable
{
    ...
    public async Task AllMutationTypes_ApplyAndSerialize()
    {
        ...
        var loaded = await store2.LoadAsync(id, CancellationToken.None);
        loaded.Should().NotBeNull();
        loaded!.Status.Should().Be(PlanStatus.Running);
        loaded.Jobs.Should().ContainKey("j1");
        loaded.Jobs.Should().NotContainKey("j2");
        loaded.Jobs["j1"].Status.Should().Be(JobStatus.Running);
        loaded.Jobs["j1"].Attempts.Should().HaveCount(1);
    }
    ...
    public async Task ReadJournal_WithFromSeq_SkipsEarlyEntries()
    {
        ...
        tail.Should().HaveCountLessThan(4);
        tail.All(m => m.Seq >= 2).Should().BeTrue();
    }
    ...
    public void IdempotencyKey_ValueEquality()
    {
        ...
        a.ToString().Should().Be(g.ToString("N"));
    ...
    public async Task Checkpoint_ReloadProducesIdenticalSnapshot()
    {
        ...
        await using var store2 = this.CreateStore();
        var loaded = await store2.LoadAsync(id, CancellationToken.None);
        loaded!.Jobs["j"].Status.Should().Be(JobStatus.Succeeded);
    }

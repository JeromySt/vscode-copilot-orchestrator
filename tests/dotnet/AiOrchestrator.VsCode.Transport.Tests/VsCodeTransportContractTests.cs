...
public sealed class VsCodeTransportContractTests
{
    ...
    public void TRANSPORT_SOLE_VsRef_Analyzer()
    {
        ...
        Directory.Exists(srcDir).Should().BeTrue($"expected src/dotnet under {repoRoot}");
        ...
        offenders.Should().BeEmpty(because: "INV-1: only AiOrchestrator.VsCode.Transport may reference Microsoft.VisualStudio.*");
    }
    ...
    public async Task TRANSPORT_SESSION_PerWindowIsolated()
    {
        ...
        s1.WindowId.Should().NotBe(s2.WindowId);
        ReferenceEquals(s1.Scope, s2.Scope).Should().BeFalse(because: "INV-2: per-window scopes must be distinct");

        // Disposing one session must not tear down the other's scope.
        await s1.DisposeAsync();
        s1.IsDisposed.Should().BeTrue();
        s2.IsDisposed.Should().BeFalse();
    }
    ...
    public async Task TRANSPORT_SESSION_IdleTimeoutDisposes()
    {
        ...
        session.IsDisposed.Should().BeTrue(because: "INV-3: idle timeout must auto-dispose the session");
    }
    ...
    public async Task TRANSPORT_WATCH_EventsMultiplexed()
    {
        ...
        kinds.Should().BeEquivalentTo(new[] { "plan.progress", "job.state", "log" },
            because: "INV-4: events of different kinds must be multiplexed through a single iterator");
    }
    ...
    public async Task TRANSPORT_TOOL_RoutesThroughMcp()
    {
        ...
        tool.Invocations.Should().Be(1, because: "INV-5: invocations must travel through the MCP registry");
    ...
    public async Task TRANSPORT_PROGRESS_NotifiesViaMcp()
    {
        ...
        enqueued.Should().BeTrue(because: "INV-6: progress events are forwarded through the MCP bridge");

        // Verify the event was queued on the iterator (integration side of INV-6).
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(3));
        await foreach (TransportEvent evt in session.WatchEventsAsync(cts.Token))
        {
            evt.Kind.Should().Be("plan.progress");
            break;
        }
    }
    ...
    public async Task TRANSPORT_CANCELLATION_Propagates()
    {
        ...
        await act.Should().ThrowAsync<OperationCanceledException>(
            because: "INV-7: cancellation from VS Code must propagate to the orchestrator");
    }
    ...
    public async Task TRANSPORT_ERROR_PreservesDotnetTypeName()
    {
        ...
        result["error"]!["code"]!.GetValue<string>().Should()
            .Be("InvalidOperationException",
                because: "INV-8: the .NET exception type name must survive the boundary verbatim");
    }

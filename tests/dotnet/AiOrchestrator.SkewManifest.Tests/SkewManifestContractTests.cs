...
public sealed class SkewManifestContractTests
{
    ...
    public async Task SKEW_MFST_1_HttpsCertificateValidated()
    {
        ...
        result.Ok.Should().BeTrue(because: result.Detail ?? "(no detail)");
        seen.Should().NotBeNull();
        seen!.RequestUri!.Scheme.Should().Be("https");
    }
    ...
    public async Task SKEW_MFST_2_ExpiredManifestRejected()
    {
        ...
        verification.Reason.Should().Be(SkewManifestRejectionReason.ExpiredManifest);
    }
    ...
    public async Task SKEW_MFST_3_StaleAfter30DaysWarns()
    {
        ...
        bus.Published.OfType<SkewManifestStale>().Should().NotBeEmpty();
    }
    ...
    public async Task OFFLINE_ROOT_1_MofNRequired()
    {
        ...
        v.Ok.Should().BeFalse();
        v.Reason.Should().Be(SkewManifestRejectionReason.InsufficientSignatures);
    }
    ...
    public async Task OFFLINE_ROOT_2_BurnInHsmKeysImmutable()
    {
        ...
        v.Ok.Should().BeFalse();
        v.Reason.Should().Be(SkewManifestRejectionReason.UnknownHsmSigner);
    }
    ...
    public async Task OFFLINE_ROOT_3_VersionRegressionRejected()
    {
        ...
        v.Ok.Should().BeFalse();
        v.Reason.Should().Be(SkewManifestRejectionReason.VersionRegression);
    }
    ...
    public async Task OFFLINE_ROOT_4_EmergencyRevocationRequiresSeparateHsmSigs()
    {
        ...
        v.Ok.Should().BeFalse();
        v.Reason.Should().Be(SkewManifestRejectionReason.EmergencyRevocationInvalid);
    }
    ...
    public async Task OFFLINE_ROOT_5_TransparencyLogInclusionVerified()
    {
        ...
        var v = await observer.VerifyAsync(manifest, CancellationToken.None);
        v.Ok.Should().BeTrue(because: v.Detail ?? "(no detail)");
    }
    ...
    public void OFFLINE_ROOT_7_DaemonNeverSignsManifest()
    {
        ...
        offendingMethods.Should().BeEmpty(because: "Daemon assembly must never expose a Sign* method (INV-10).");
    }
    ...
    public async Task SKEW_MFST_AUDITED_OnEveryFetch()
    {
        ...
        audit.Records.Should().OnlyContain(r => r.EventType.Contains("skew.manifest", StringComparison.Ordinal));
    }
    ...
    public void KEY_CEREMONY_STUB_NotCalledFromProdAssemblies()
    {
        ...
        callable.Should().BeEmpty(because: "KeyCeremonyToolingStub exposes zero callable public methods.");

        var publicCtors = stubType.GetConstructors(BindingFlags.Public | BindingFlags.Instance);
        publicCtors.Should().BeEmpty(because: "KeyCeremonyToolingStub exposes no public constructors.");
    }

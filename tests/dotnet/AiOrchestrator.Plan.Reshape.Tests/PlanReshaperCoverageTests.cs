...
public sealed class PlanReshaperCoverageTests : IAsyncLifetime
{
    ...
    public async Task AddJob_InvalidSpec_Fails()
    {
        ...
        ex.Which.Failures.Single().FailureReason.Should().Be("InvalidSpec");
    }
    ...
    public async Task AddJob_DuplicateId_Fails()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(planId, ImmutableArray.Create<ReshapeOperation>(op), IdempotencyKey.FromGuid(Guid.NewGuid()), default);
        var ex = await act.Should().ThrowAsync<ReshapeRejectedException>();
        ex.Which.Failures.Single().FailureReason.Should().Be("DuplicateJobId");
    }
    ...
    public async Task AddJob_UnknownDependency_Fails()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(planId, ImmutableArray.Create<ReshapeOperation>(op), IdempotencyKey.FromGuid(Guid.NewGuid()), default);
        var ex = await act.Should().ThrowAsync<ReshapeRejectedException>();
        ex.Which.Failures.Single().FailureReason.Should().Be("UnknownDependency");
    }
    ...
    public async Task RemoveJob_UnknownJob_Fails()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(planId, ImmutableArray.Create<ReshapeOperation>(op), IdempotencyKey.FromGuid(Guid.NewGuid()), default);
        var ex = await act.Should().ThrowAsync<ReshapeRejectedException>();
        ex.Which.Failures.Single().FailureReason.Should().Be("UnknownJob");
    }
    ...
    public async Task RemoveJob_Succeeds_OnPending()
    {
        ...
        result.PerOperation.Single().Success.Should().BeTrue();
        var loaded = await this.store.LoadAsync(planId, default);
        loaded!.Jobs.Should().NotContainKey(a.ToString());
    }
    ...
    public async Task UpdateDeps_Succeeds()
    {
        ...
        result.PerOperation.Single().Success.Should().BeTrue();
        var loaded = await this.store.LoadAsync(planId, default);
        loaded!.Jobs[b.ToString()].DependsOn.Should().ContainSingle().Which.Should().Be(a.ToString());
    }
    ...
    public async Task AddBefore_Succeeds_PrependsNewDependency()
    {
        ...
        loaded.Jobs[a.ToString()].DependsOn.Should().Contain(newId.ToString());
    }
    ...
    public async Task AddBefore_ExistingNotPending_Fails()
    {
        ...
        var act = async () => await reshaper.ApplyAsync(planId, ImmutableArray.Create<ReshapeOperation>(op), IdempotencyKey.FromGuid(Guid.NewGuid()), default);
        var ex = await act.Should().ThrowAsync<ReshapeRejectedException>();
        ex.Which.Failures.Single().FailureReason.Should().Be("StatusNotUpdatable");
    }
    ...
    public void CycleResult_Records_ShapeIsAsExpected()
    {
        ...
        cycleFalse.Cycle.Should().BeFalse();
        cycleFalse.Cycle_.Should().BeNull();

        var ids = ImmutableArray.Create(JobId.New(), JobId.New());
        var cycleTrue = new CycleResult { Cycle = true, Cycle_ = ids };
        cycleTrue.Cycle.Should().BeTrue();
        cycleTrue.Cycle_.Should().NotBeNull();
        cycleTrue.Cycle_!.Value.Length.Should().Be(2);
    }
    ...
    public async Task DagLim_MaxJobs_Default_IsUnbounded()
    {
        ...
        result.PerOperation.Should().OnlyContain(r => r.Success);
    }
}

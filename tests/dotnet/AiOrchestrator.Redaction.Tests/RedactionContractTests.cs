...
public sealed class RedactionContractTests
{
    ...
    public void T3Red1_GitHubPat_IsRedacted()
    {
        ...
        matches[0].RuleId.Should().Be("T3-RED-1");
        matches[0].Length.Should().BeGreaterThanOrEqualTo(pat.Length);
    }
    ...
    public void T3Red2_AwsAccessKey_IsRedacted()
    {
        ...
        matches.Should().HaveCount(1);
        matches[0].RuleId.Should().Be("T3-RED-2");
        matches[0].Length.Should().Be(awsKey.Length);
    }
    ...
    public void T3Red3_BearerToken_IsRedacted()
    {
        ...
        matches.Should().AllSatisfy(m => m.RuleId.Should().Be("T3-RED-3"));
    }
    ...
    public void T3Red4_ConnectionStringPassword_IsRedacted()
    {
        ...
        matches.Should().NotBeEmpty();
        matches.Should().AllSatisfy(m => m.RuleId.Should().Be("T3-RED-4"));
    }
    ...
    public void T3Red5_SshPrivateKeyHeader_IsRedacted()
    {
        ...
        matches.Should().HaveCount(1);
        matches[0].RuleId.Should().Be("T3-RED-5");
    }
    ...
    public void T3Red6_GenericSecret_IsRedacted()
    {
        ...
        matches.Should().HaveCount(1);
        matches[0].RuleId.Should().Be("T3-RED-6");
    }
    ...
    public void T3Red7_Jwt_IsRedacted()
    {
        ...
        matches.Should().HaveCount(1);
        matches[0].RuleId.Should().Be("T3-RED-7");
    }
    ...
    public void PSid2_WindowsUserPath_IsDetectedAndAnchored()
    {
        ...
        matches.Should().NotBeEmpty("a standalone Windows user-profile path must be detected");
        matches.Should().AllSatisfy(m => m.RuleId.Should().Be("P-SID-2"));
        ...
        noMatch.Should().BeEmpty("negative-lookbehind anchor must suppress mid-word match (INV-5)");
    }
    ...
    public void Inv3_Redact_IsIdempotent()
    {
        ...
        twice.Should().Be(once, "redacting already-redacted output must be a no-op (INV-3)");
    }
    ...
    public void Inv4_RedactedOutput_LengthNonIncrease()
    {
        ...
        foreach (var input in cases)
        {
            ...
            output.Length.Should().BeLessThanOrEqualTo(
                input.Length,
    ...
    public void Inv7_AnonymousMode_IsDeterministic()
    {
        ...
        first.Should().Be(second, "same path + same salt must always yield the same pseudonym (INV-7)");
        ...
        different.Should().NotBe(first, "different bundle salts must produce different pseudonyms (INV-7)");
    }
    ...
    public void Inv2_NoNewRegexInSource()
    {
        ...
        dir.Should().NotBeNull("repo root with src/dotnet must be locatable");
        var redactionSrc = Path.Combine(dir!.FullName, "src", "dotnet", "AiOrchestrator.Redaction");
        Directory.Exists(redactionSrc).Should().BeTrue("AiOrchestrator.Redaction source directory must exist");
        ...
        violations.Should().BeEmpty("INV-2: no source file may use new Regex() — use [GeneratedRegex] instead");
    }
}

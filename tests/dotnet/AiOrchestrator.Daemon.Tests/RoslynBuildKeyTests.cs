...
public sealed class RoslynBuildKeyTests
{
    ...
    public void DAEMON_NEVER_SIGNS_WithBuildKey()
    {
        ...
        File.Exists(asmPath).Should().BeTrue($"daemon assembly not co-located: {asmPath}");
        ...
        foreach (var handle in md.MemberReferences)
        {
            ...
            if (string.Equals(typeName, "Ed25519Signer", StringComparison.Ordinal)
        ...
        violations.Should().BeEmpty("daemon assembly must not reference Ed25519Signer.Sign (TRUST-ROOT-4)");
    }
}

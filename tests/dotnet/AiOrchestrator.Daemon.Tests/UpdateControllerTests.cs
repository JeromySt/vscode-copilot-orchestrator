...
public sealed class UpdateControllerTests : IDisposable
{
    ...
    public async Task UPD_RB_1_PollsAtConfiguredInterval()
    {
        ...
        controller.CheckCount.Should().BeGreaterThanOrEqualTo(2);
    }
    ...
    public async Task UPD_RB_2_RejectsManifestWithBadSignature()
    {
        ...
        outcome.Should().Be(UpdateOutcome.Rejected_BadSignature);
        audit.Records.Should().Contain(r => r.EventType == "DaemonUpdateRejected");
    }
    ...
    public async Task UPD_RB_3_HealthFailureTriggersRollback()
    {
        ...
        outcome.Should().Be(UpdateOutcome.RolledBack);
        audit.Records.Should().Contain(r => r.EventType == "DaemonUpdateRolledBack");
        File.ReadAllText(Path.Combine(installRoot.Value, "marker.txt")).Should().Be("v1");
    }
    ...
    public async Task UPD_RB_4_VersionRegressionRejected()
    {
        ...
        outcome.Should().Be(UpdateOutcome.Rejected_VersionRegression);
        audit.Records.Should().Contain(r => r.EventType == "DaemonUpdateRejected");
    }
    ...
    public async Task UPD_RB_4_DowngradeBlocked()
    {
        ...
        outcome.Should().Be(UpdateOutcome.Rejected_DowngradeBlocked);
        audit.Records.Should().Contain(r => r.EventType == "DaemonUpdateRejected");
    }
    ...
    public async Task UPD_RB_5_AuditedOnEveryOutcome()
    {
        ...
        (await controller.CheckAndApplyAsync(CancellationToken.None)).Should().Be(UpdateOutcome.Applied);
        audit.Records.Should().Contain(r => r.EventType == "DaemonUpdateApplied");
        ...
        (await controller.CheckAndApplyAsync(CancellationToken.None)).Should().Be(UpdateOutcome.Rejected_VersionRegression);

        audit.Records.Select(r => r.EventType).Distinct().Should().Contain(new[] { "DaemonUpdateApplied", "DaemonUpdateRejected" });
    }
    ...
    public async Task UPD_RB_6_BuildKeyRolloverObservedOnNewKeys()
    {
        ...
        bus.Published.OfType<BuildKeyRolloverObserved>().Should().BeEmpty("first observation just records baseline");

        http.Responses[optsMon.CurrentValue.ReleaseManifestUrl] = Encoding.UTF8.GetBytes(jsonB);
        await controller.CheckAndApplyAsync(CancellationToken.None);
        bus.Published.OfType<BuildKeyRolloverObserved>().Should().HaveCount(1);
    }
    ...
    public async Task UPD_RB_7_PidFileAtomicAndConflictDetected()
    {
        ...
        fs.Files.Should().ContainKey(path.Value);
        fs.Files.Keys.Where(k => k.Contains(".tmp-", StringComparison.Ordinal)).Should().BeEmpty();
        Encoding.UTF8.GetString(fs.Files[path.Value]).Trim().Should().Be("12345");
        ...
        await FluentActions.Awaiting(() => writer.AcquireOrThrowAsync(path, CancellationToken.None).AsTask())

// <copyright file="T22T14ResolverTests.cs" company="AiOrchestrator contributors">
...
public sealed class T22T14ResolverTests
{
    ...
    public void T22_RULE_SvInjectionAfterLeavesReady()
    {
        ...
        svNode.DependsOn.Should().BeEquivalentTo(
            new[] { leaf1.Id, leaf2.Id },
            "T22-RULE: SV.DependsOn must equal all leaf jobs (those with no successors)");

        result.SvDependencyEdges.Should().HaveCount(2, "one edge per leaf job");
        result.SvDependencyEdges.All(e => e.To == JobId.Parse(svId)).Should().BeTrue("all edges point to SV");
    }
    ...
    public void T14_RULE_ReshapeReinjectsSvDeps()
    {
        ...
        var svNode = result.AdjustedPlan.Jobs[svId];
        svNode.DependsOn.Should().BeEquivalentTo(
            new[] { existingLeaf.Id, addedJob.Id },
            "T14-RULE: adding a new leaf job forces SV.DependsOn to include the new leaf");

        result.SvDependencyEdges.Should().HaveCount(2, "two leaf→SV edges after reshape");
    }
    ...
    public void T22_T14_FUZZ_RandomReshapeNeverLosesSv()
    {
        ...
        for (int i = 0; i < 10_000; i++)
        {
            ...
            svDeps.Should().Equal(
                expectedLeafIds,
                $"iteration {i}: SV.DependsOn must equal the set of all non-SV job IDs");
        }
    }
}

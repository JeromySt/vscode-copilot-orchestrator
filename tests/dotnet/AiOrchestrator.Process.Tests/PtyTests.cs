...
public sealed class PtyTests
{
    ...
    public async Task PROC_7_Pty_DeliversInteractivePrompt()
    {
            ...
            RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            ...
            pty.Master.Should().NotBeNull();
            pty.Slave.Should().NotBeNull();
            pty.Master.IsInvalid.Should().BeFalse("master handle must be valid");
            pty.Slave.IsInvalid.Should().BeFalse("slave handle must be valid");
        }

...
public sealed class ResourceLimitsTests
{
    ...
    public async Task PROC_9_RLimits_AppliedBeforeExec_Linux()
    {
        ...
        result.Should().Be(0, "getrlimit should succeed");
        currentLimits.RlimCur.Should().BeGreaterThan(0);

        // Verify RLimitsLinux.Apply doesn't throw (best we can do without a child process fixture)
        var pid = System.Diagnostics.Process.GetCurrentProcess().Id;
        var act = () => RLimitsLinux.Apply(pid, limits);
        act.Should().NotThrow("applying resource limits to a valid PID must not throw");
    }
    ...
    public async Task PROC_10_JobObjects_LimitMemory_Windows()
    {
        ...
        var act = () => ProcessSpawner.ApplyLimits(handle.ProcessId, limits);
        act.Should().NotThrow("applying job object limits to a valid PID must not throw");

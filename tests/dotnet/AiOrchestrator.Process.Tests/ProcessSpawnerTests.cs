...
public sealed class FakeProcessSpawnerContractTests : ProcessSpawnerContractTests
{
    ...
    public async Task PROC_11_FakeProcessSpawner_SatisfiesContract()
    {
        ...
        handle.ProcessId.Should().BeGreaterThan(0);
        handle.StandardOut.Should().NotBeNull();
        handle.StandardError.Should().NotBeNull();
        handle.StandardIn.Should().NotBeNull();
        _spawner.SpawnedHandles.Should().HaveCount(1);
    }
}
...
public sealed class RealProcessSpawnerTests
{
    ...
    public async Task PROC_1_Spawn_UsesArgvVector_NotShell()
    {
        ...
        exitCode.Should().Be(0, "the echo command should succeed without shell parsing");
    }
    ...
    public async Task PROC_2_Cancellation_SendsSigtermThenSigkill()
    {
        ...
        completed.Should().Be(waitTask, "process should be killed within the grace+kill window");
    }
    ...
    public async Task PROC_3_EnvScope_BlocksInheritedSecrets()
    {
        ...
        try
        {
            ...
            output.Should().NotContain(SecretKey,
                "the secret env var should not leak to the child process");
            output.Should().Contain(AllowedKey,
                "the explicitly allowed var should be present");
        }
        finally
        {

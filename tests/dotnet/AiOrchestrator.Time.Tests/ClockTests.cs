...
public sealed class ClockTests
{
    ...
    public void CLOCK_1_UtcNow_ReturnsUtc()
    {
        ...
        clock.UtcNow.Offset.Should().Be(TimeSpan.Zero, "IClock.UtcNow must be UTC");
    }
    ...
    public void CLOCK_3_MonotonicGuard_DetectsRegression_Throws()
    {
        ...
        guard.MonotonicMilliseconds.Should().Be(1000);

        // Regress the inner clock below the baseline
        inner.SetMonotonicMs(500);

        // Guard must throw and record telemetry
        var ex = guard.Invoking(g => g.MonotonicMilliseconds)
            .Should()
            .Throw<MonotonicRegressionException>()
            .Which;

        ex.Previous.Should().Be(1000);
        ex.Current.Should().Be(500);

        telemetry.Counters.Should().ContainKey("MonotonicClockRegression");
        telemetry.Counters["MonotonicClockRegression"].Should().Be(1);
    }
    ...
    public async Task CLOCK_4_DelayProvider_HonorsCancellationWithin50ms()
    {
        ...
        threw.Should().BeTrue("cancellation must propagate as OperationCanceledException");
        sw.ElapsedMilliseconds.Should().BeLessThan(50, "cancellation must be honored within 50 ms");
    }
    ...
    public void CLOCK_5_DefaultRegistration_IsMonotonicGuardWrappingSystemClock()
    {
        ...
        clock.Should().BeOfType<MonotonicGuard>(
            "AddTime() must register IClock as MonotonicGuard wrapping SystemClock");

        var delay = sp.GetRequiredService<IDelayProvider>();
        delay.Should().BeOfType<SystemDelayProvider>(
            "AddTime() must register IDelayProvider as SystemDelayProvider");
    }

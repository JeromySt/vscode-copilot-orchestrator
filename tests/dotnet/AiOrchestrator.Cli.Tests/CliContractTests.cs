...
public sealed class CliContractTests
{
    ...
    public void CLI_VERB_REGISTERED_All15()
    {
        ...
        handlers.Select(h => h.VerbPath).Should().BeEquivalentTo(ExpectedVerbs);

        RootCommand root = Program.BuildCommandTree(sp);
        root.Subcommands.Select(c => c.Name).Should().Contain(new[] { "plan", "daemon", "version" });

        Command plan = root.Subcommands.Single(c => c.Name == "plan");
        plan.Subcommands.Select(c => c.Name).Should().BeEquivalentTo(new[]
        {
        ...
        daemon.Subcommands.Select(c => c.Name).Should().BeEquivalentTo(new[] { "start", "stop", "status" });
    }
    ...
    public void CLI_VERB_HelpSnapshotsPass()
    {
        ...
        Directory.Exists(snapsDir).Should().BeTrue(because: $"snapshots dir must exist: {snapsDir}");
        ...
        foreach (ICliVerbHandler h in Program.EnumerateHandlers(sp))
        {
            ...
            File.Exists(snap).Should().BeTrue(because: $"snapshot for '{h.VerbPath}' must exist at {snap}");
            string stored = Normalize(File.ReadAllText(snap));
            produced.Should().Be(stored, because: $"help for '{h.VerbPath}' must match snapshot");
        }
    }
    ...
    public void CLI_JSON_SourceGenSerializerOnly()
    {
        ...
        offenders.Should().BeEmpty(because: "INV-4: all JsonSerializer.Serialize calls must pass source-gen JsonTypeInfo");
    }
    ...
    public void CLI_NO_COLOR_RespectsEnv(bool noColor, string? env, bool expectedColor)
    {
        ...
        w.ColorEnabled.Should().Be(expectedColor);
    }
    ...
    public async Task CLI_PLAN_RUN_BlocksUntilTerminal()
    {
        ...
        (end - start).TotalMilliseconds.Should().BeGreaterThanOrEqualTo(100, because: "must not return before the probe completes");
    }
    ...
    public async Task CLI_PATH_ARG_ValidatedThroughIPathValidator()
    {
        ...
        try
        {
            ...
            exit.Should().Be(CliExitCodes.Ok);
            validator.Asserted.Should().NotBeEmpty(because: "IPathValidator.AssertSafe must be invoked for --out");
            validator.Asserted.Last().Should().EndWith("bundle.tgz");
        }
        finally
        {
            try

...
public sealed class EventBusContractTests
{
    ...
    public void EVT_AUTH_1_FilterRespectsSubscribePrincipal()
    {
        ...
        sut.Matches(filter, envelope, currentPrincipal: MakePrincipal("mallory")).Should().BeTrue();
    }
    ...
    public void EVT_AUTH_2_PrincipalChangeDoesNotEscalate()
    {
        ...
        sut.Matches(filter, envelope, current).Should().BeFalse(
            "filter must evaluate against pinned principal, not current principal");
    }
    ...
    public void EVT_AUTH_3_FilterRejectsCrossPrincipalReplay()
    {
        ...
        sut.Matches(filter, envelope, currentPrincipal: pinned).Should().BeFalse();
    }
    ...
    public async Task EVT_BUS_1_Publish_AppliesRedactor()
    {
        ...
        got.Message.Should().Be("password=***");
        redactor.Calls.Should().BeGreaterThan(0);
    ...
    public async Task EVT_BUS_2_Backpressure_Wait_BlocksPublisher()
    {
        ...
        winner.Should().NotBe(blocked, "Wait mode must block publisher when buffer is full");
    ...
    public async Task EVT_BUS_3_Backpressure_DropOldest_EmitsLaggedEvent()
    {
        ...
        lag.Mode.Should().Be(BackpressureMode.DropOldest);
        lag.DroppedCount.Should().BeGreaterThan(0);
    ...
    public async Task EVT_BUS_4_Subscribe_ReturnsAsyncDisposable_DrainsOnDispose()
    {
        ...
        await sub.DisposeAsync();
        seen.Count.Should().Be(10, "dispose must drain the buffered events");
    }
    ...
    public async Task CONC_CHAN_2_DedupesByEventKey()
    {
        ...
        seen.Count.Should().Be(2, "second publish of k1 must be deduped within window");
    }
    ...
    public void EVT_BUS_5_NoLocksOnPublishPath()
    {
        ...
        publishBlock.Should().NotContain("lock(");
        publishBlock.Should().NotContain("Monitor.Enter");
        publishBlock.Should().NotContain("SemaphoreSlim");
    }
    ...
    public void AuthContextFilter_NullPinned_ReturnsFalse()
    {
        ...
        sut.Matches(filter, MakeEnvelope("alice"), MakePrincipal("alice")).Should().BeFalse();
    }
    ...
    public void AuthContextFilter_PlanIdMismatch_ReturnsFalse()
    {
        ...
        sut.Matches(filter, env, MakePrincipal("alice")).Should().BeFalse();
    }
    ...
    public void AuthContextFilter_NullEnvelopePrincipal_Allowed()
    {
        ...
        sut.Matches(filter, MakeEnvelope(principalId: null), MakePrincipal("alice")).Should().BeTrue();
    }
    ...
    public async Task PublishAsync_BackpressureThrow_RaisesEventBusFullException()
    {
        ...
        await act.Should().ThrowAsync<EventBusFullException>();
        hold.SetResult();
    }
    ...
    public async Task DedupCache_AfterWindow_AllowsRepublish()
    {
        ...
        await sub.DisposeAsync();
        seen.Count.Should().Be(2, "after window expiry the same key may be republished");
    }
    ...
    public async Task PublishAsync_HandlerThrows_ReaderLoopContinues()
    {
        ...
        await sub.DisposeAsync();
        seen.Should().BeGreaterThanOrEqualTo(2);
    }

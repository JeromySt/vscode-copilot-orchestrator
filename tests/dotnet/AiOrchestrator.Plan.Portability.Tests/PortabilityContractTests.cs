...
public sealed class PortabilityContractTests : IDisposable
{
    ...
    public async Task PORT_1_BundleSharesSchemaWithDiagnose()
    {
        ...
        {
            root.TryGetProperty(required, out _).Should().BeTrue($"manifest must expose shared key '{required}'");
        }

        root.GetProperty("kind").GetString().Should().Be("plan");
    ...
    public async Task PORT_2_PlanContentDeterministic()
    {
        ...
        plan1.Should().Be(plan2, "plan JSON must be byte-stable from PlanSerializationContext (PORT-2)");
    }
    ...
    public async Task PORT_3_AbsolutePathsRedacted()
    {
        ...
        planJson.Should().NotContain(absolutePath, "RedactPaths=true must strip absolute prefixes (PORT-3)");
    }
    ...
    public async Task PORT_4_SchemaMismatchThrows()
    {
        ...
        await act.Should().ThrowAsync<PortabilitySchemaMismatchException>();
    }
    ...
    public async Task PORT_5_GenerateNewIdOnConflict()
    {
        ...
        newId.Should().NotBe(planId, "GenerateNewId must assign a fresh PlanId on collision (PORT-5)");
    ...
    public async Task PORT_5_RejectOnConflict()
    {
        ...
        var ex = await act.Should().ThrowAsync<ImportConflictException>();
        ex.Which.ExistingPlanId.Should().Be(planId);
    }
    ...
    public async Task PORT_5_OverwriteIfArchivedOnly()
    {
        ...
        var act = async () => await importer.ImportAsync(
            ...
            CancellationToken.None);
        (await act.Should().ThrowAsync<ImportConflictException>()).Which.ExistingStatus.Should().Be(PlanStatus.Pending);
        ...
        var newId = await importer2.ImportAsync(
            ...
            new ImportOptions { IfPlanIdExists = ImportConflictPolicy.OverwriteIfArchived },
            CancellationToken.None);
        newId.Should().NotBe(default(PlanId));
    }
    ...
    public async Task PORT_6_AttemptsStrippedWhenFlag()
    {
        ...
        strippedJson.Should().NotContain("\"attemptNumber\"", "attempts must be stripped when IncludeAttempts=false (PORT-6)");
        ...
        keptJson.Should().Contain("\"attemptNumber\"", "attempts must be retained when IncludeAttempts=true");
    }
    ...
    public async Task PORT_ROUNDTRIP_BitIdentical()
    {
        ...
        normalized2.Should().Be(normalized1, "round-trip plan content must be bit-identical (INV-7)");
    }
    ...
    public void ConstructorGuards_ThrowOnNullDependencies()
    {
        ...
        ((Action)(() => new PlanExporter(null!, fs, clock, opts))).Should().Throw<ArgumentNullException>();
        ((Action)(() => new PlanExporter(store, null!, clock, opts))).Should().Throw<ArgumentNullException>();
        ((Action)(() => new PlanExporter(store, fs, null!, opts))).Should().Throw<ArgumentNullException>();
        ((Action)(() => new PlanExporter(store, fs, clock, null!))).Should().Throw<ArgumentNullException>();

        ((Action)(() => new PlanImporter(null!, fs, clock, opts))).Should().Throw<ArgumentNullException>();
        ((Action)(() => new PlanImporter(store, null!, clock, opts))).Should().Throw<ArgumentNullException>();
        ((Action)(() => new PlanImporter(store, fs, null!, opts))).Should().Throw<ArgumentNullException>();
        ((Action)(() => new PlanImporter(store, fs, clock, null!))).Should().Throw<ArgumentNullException>();
    }
    ...
    public void RedactPath_HandlesEdgeCases()
    {
        typeof(PlanExporter)
            ...
            .Invoke(null, new object?[] { string.Empty })!.Should().Be(string.Empty);

        typeof(PlanExporter)
            .GetMethod("RedactPath", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static)!
            .Invoke(null, new object?[] { "relative/path" })!.Should().Be("relative/path");
    }
    ...
    public async Task Import_WrongKind_Throws()
    {
        ...
        ((Action)(() => PlanImporter.Load(outputPath))).Should().Throw<InvalidDataException>();
    }

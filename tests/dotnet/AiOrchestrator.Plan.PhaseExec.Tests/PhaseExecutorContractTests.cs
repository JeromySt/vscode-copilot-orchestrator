...
public sealed class PhaseExecutorContractTests
{
    ...
    public async Task PHASE_ORDER_FixedSequence()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Succeeded);
        result.EndedAtPhase.Should().Be(JobPhase.Done);
        seenOrder.Should().Equal(
            JobPhase.Setup,
            JobPhase.Prechecks,
            JobPhase.Work,
            JobPhase.Postchecks,
        ...
        result.CommitSha.Should().Be(SampleSha);
    }
    ...
    public async Task HEAL_RESUME_1_TransientNetworkResumes()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Succeeded);
        result.AttemptCount.Should().Be(2, "phase-resume = a fresh attempt that re-runs from the failed phase");
        work.Calls.Count.Should().Be(2);
    }
    ...
    public async Task HEAL_RESUME_1_RemoteRejectedGivesUp()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Failed);
        result.EndedAtPhase.Should().Be(JobPhase.ForwardIntegration);
        result.AttemptCount.Should().Be(1, "RemoteRejected is non-recoverable");
        fi.Calls.Count.Should().Be(1);
    }
    ...
    public async Task HEAL_RESUME_2_DoesNotRepeatSuccessfulPhase()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Succeeded);
        setup.Calls.Count.Should().Be(1, "Setup must not be re-run on auto-heal");
        pre.Calls.Count.Should().Be(1, "Prechecks must not be re-run on auto-heal");
        work.Calls.Count.Should().Be(2);
        post.Calls.Count.Should().Be(1);
        commit.Calls.Count.Should().Be(1);
        fi.Calls.Count.Should().Be(1);
    }
    ...
    public async Task HEAL_RESUME_3_MaxAttemptsCapped()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Failed);
        result.EndedAtPhase.Should().Be(JobPhase.Work);
        result.AttemptCount.Should().Be(3, "1 initial + 2 auto-heals = 3 attempts");
        work.Calls.Count.Should().Be(3);
    }
    ...
    public async Task DISK_PLAN_1_ReservedDuringCommit()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Failed);
        result.EndedAtPhase.Should().Be(JobPhase.Commit);
        result.CommitSha.Should().BeNull();
        fi.Calls.Should().BeEmpty("FI must not run after commit-time quota failure");
    ...
    public async Task COMMIT_EXPECTS_NO_CHANGES_PassesOnEmptyDiff()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Succeeded);
        result.CommitSha.Should().BeNull("expectsNoChanges semantics produce no commit SHA");
    }
    ...
    public async Task COMMIT_INVOKES_HOOKGATE_PreCommit()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Succeeded);
        order.Should().ContainInOrder("hookgate", "git");
    }
    ...
    public async Task PHASE_CANCELLATION_AbortsWithin1Sec()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Canceled);
    ...
    public async Task PHASE_LEASE_HeldThroughForwardIntegration()
    {
        ...
        result.FinalStatus.Should().Be(JobStatus.Succeeded);
        seen.First().Should().Be(JobPhase.Setup);
        seen.Last().Should().Be(JobPhase.ForwardIntegration);
    ...
    public async Task RECORD_ATTEMPT_PerAttemptMutation()
    {
        ...
        store.Recorded.Should().HaveCount(1);
        store.Recorded[0].Attempt.Status.Should().Be(JobStatus.Succeeded);
        bus.Of<PhaseAttemptCompletedEvent>().Should().HaveCount(1);
    }

...
public sealed class HostConcurrencyBrokerTests
{
    ...
    public void CONC_BROKER_2_LinuxUdsIsPathBased()
    {
        ...
        opts.SocketPath.Value.Should().NotStartWith("\0",
            "UDS path must be filesystem-based per CONC-BROKER-2, not an abstract socket");
        opts.SocketPath.Value.Should().StartWith("/",
            "UDS socket must be a POSIX absolute path");

        // Verify directory component is non-trivial.
        var dir = Path.GetDirectoryName(opts.SocketPath.Value);
        dir.Should().NotBeNullOrEmpty("UDS socket must reside in a directory");

        // Explicit non-abstract: the default path must not use '@abstract' notation.
        opts.SocketPath.Value.Should().NotContain("@abstract",
            "abstract sockets are prohibited per CONC-BROKER-2 v1.4");
    }
    ...
    public async Task CONC_BROKER_3_PeerCredsCheckedPerMessage()
    {
        ...
        admission.Principal.PrincipalId.Should().Be("peer-creds-user");
    ...
    public async Task CONC_BROKER_FAIR_1_ProportionalDivision()
    {
        ...
        for (var u = 0; u < UsersCount; u++)
        {
            admissionCounts[u].Should().BeInRange(30 - 7, 37,
                $"user {u + 1} should receive approximately 1/3 of admissions in proportional mode");
        }
    }
    ...
    public async Task CONC_BROKER_FAIR_2_StrictRoundRobinDeterministic()
    {
        ...
        admissionOrder.Should().HaveCount(3, "all three requests should be admitted");
        admissionOrder[0].Should().Be("rr-user-1", "first admitted is p1");
        admissionOrder.Skip(1).Should().Contain("rr-user-2", "p2 should be admitted");
        admissionOrder.Skip(1).Should().Contain("rr-user-3", "p3 should be admitted");
    }
    ...
    public async Task CONC_BROKER_HostQueued_HasActionableHintAndEta()
    {
        ...
        queuedEvents.Should().HaveCount(1, "one queued event should be emitted");

        var evt = queuedEvents[0];
        evt.ActionableHint.Should().NotBeNullOrEmpty("ActionableHint must be non-empty (CONC-BROKER-HINT)");
        evt.EtaSeconds.Should().BeGreaterThanOrEqualTo(TimeSpan.Zero, "EtaSeconds must be non-negative");
    ...
    public async Task CONC_BROKER_LeaseExpiresAfterTtl()
    {
        ...
        expiredEvents.Should().HaveCount(1, "one HostAdmissionExpired event should be published");
        expiredEvents[0].BrokerLeaseId.Should().Be(admission.BrokerLeaseId);
        ...
        second.Principal.PrincipalId.Should().Be("ttl-user-2");
        await second.DisposeAsync();
    ...
    public async Task CONC_BROKER_GracefulShutdown_DrainsInFlight()
    {
        ...
        ex.Should().Match<Exception>(e =>
            e is OperationCanceledException || e is BrokerUnavailableException,
    ...
    public async Task CONC_BROKER_UnavailableFallsBackToUserOnly()
    {
        ...
        admission.Should().NotBeNull("client must return an admission even when broker is unavailable");
        admission.Principal.PrincipalId.Should().Be("fallback-user");
        admission.BrokerLeaseId.Should().StartWith("passthrough-");

        // Warning should have been logged once.
        logger.Warnings.Should().HaveCount(1, "exactly one warning should be logged on first unavailable call");
        ...
        logger.Warnings.Should().HaveCount(1, "warning is only logged once");
    ...
    public async Task MultipleExpiresAreFiredForMultipleLeases()
    {
        ...
        bus.Of<HostAdmissionExpired>().Should().HaveCount(2, "both leases should expire");
    ...
    public async Task SameUserMultipleQueuedJobsGetsSameUserHint()
    {
        ...
        events.Should().Contain(e => e.ActionableHint.Contains("same user"),
            "queued events from same principal should mention same-user hint");
    ...
    public async Task FairnessSchedulerDrainReturnsAllWaiters()
    {
        ...
        waiters.Should().HaveCount(1, "one request is pending in the queue");

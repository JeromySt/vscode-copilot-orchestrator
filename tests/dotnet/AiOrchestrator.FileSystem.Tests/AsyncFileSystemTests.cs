...
public sealed class AsyncFileSystemTests
{
    ...
    public async Task CMD_TMP_1_NewFileUsesOExclAnd0600_Linux()
    {
        ...
        ((int)mode & 0x1FF).Should().Be(0x180, "file should be created with mode 0600");
    }
    ...
    public async Task CMD_TMP_1_NewFileUsesCreateNewAndOwnerOnlyDacl_Windows()
    {
        ...
        rules.Count.Should().BeGreaterThan(0, "DACL should have at least one explicit rule (owner-only)");
    }
    ...
    public async Task FS_2_AtomicMove_PreservesPermissions()
    {
        ...
        (await fs.ExistsAsync(src, default)).Should().BeFalse("source must be gone after rename");
        (await fs.ExistsAsync(dst, default)).Should().BeTrue("destination must contain the moved file");
        (await fs.ReadAllTextAsync(dst, default)).Should().Be("atomic");
    }
    ...
    public async Task OpenReadAsync_StreamsContent()
    {
        ...
        (await reader.ReadToEndAsync()).Should().Be("hello-stream");
    }
    ...
    public async Task DeleteAsync_RemovesDirectoryAndIsIdempotent()
    {
        ...
        Directory.Exists(dir.Value).Should().BeFalse();
    ...
    public async Task GetMountKindAsync_DelegatesToInspector()
    {
        ...
        kind.Should().BeOneOf(MountKind.Local, MountKind.Unknown, MountKind.Smb);
    }
    ...
    public async Task ExistsAsync_ReturnsTrueForExistingFile()
    {
        ...
        (await fs.ExistsAsync(target, default)).Should().BeTrue();
    }

...
suite('Plan Builder', () => {
  ...
  suite('buildPlan()', () => {
    test('creates plan with single node', () => {
      ...
      assert.strictEqual(plan.jobs.size, 2);
    ...
    test('creates plan with diamond dependency', () => {
      ...
      assert.strictEqual(plan.jobs.size, 5);
      assert.strictEqual(plan.roots.length, 1);
      assert.strictEqual(plan.leaves.length, 1);
    });
    ...
    test('computes dependents (reverse edges)', () => {
      const plan = buildPlan({ name: 'Test', jobs: [makeJob('a'), makeJob('b', ['a'])] });
      const rootNode = plan.jobs.get(plan.roots[0])!;
  ...
  suite('validation errors', () => {
    ...
    test('accepts empty jobs array (SV node is auto-injected)', () => {
      // Empty jobs array is valid because SV node is always injected
      const plan = buildPlan({ name: 'Test', jobs: [] });
      assert.strictEqual(plan.jobs.size, 1); // Just the SV node
      assert.ok(plan.producerIdToNodeId.has('__snapshot-validation__'));
    });
    ...
    test('PlanValidationError has correct name', () => {
      try {
        // Use duplicate producerId to trigger validation error
        buildPlan({ name: 'Test', jobs: [makeJob('dup'), makeJob('dup')] });
...
suite('buildSingleJobPlan', () => {
  ...
  test('generates unique UUIDs for id and planId', () => {
    ...
    const node1 = [...plan1.jobs.values()][0];
    const node2 = [...plan2.jobs.values()][0];
    assert.notStrictEqual(node1.id, node2.id);
  });

  test('uses default maxParallel from buildPlan (0 = unlimited)', () => {
    const plan = buildSingleJobPlan({ name: 'Build', task: 'build' });
...
suite('buildPlan with groups', () => {
  ...
  test('sets maxParallel default 0 (unlimited)', () => {
    const plan = buildPlan({ name: 'T', jobs: [makeJob('a')] });
    assert.strictEqual(plan.maxParallel, 0);
  });
...
suite('Complex dependency scenarios', () => {
  ...
  test('long chain dependency', () => {
    ...
    assert.strictEqual(plan.jobs.size, 6);
    assert.strictEqual(plan.roots.length, 1);
    assert.strictEqual(plan.leaves.length, 1);
    
    // Verify the chain
    const nodeE = [...plan.jobs.values()].find(n => n.producerId === 'e')!;
    const nodeD = [...plan.jobs.values()].find(n => n.producerId === 'd')!;
  ...
  test('complex multi-root multi-leaf structure', () => {
    ...
    assert.strictEqual(plan.jobs.size, 7);
  ...
  test('node with multiple dependencies becomes ready only when all deps are done', () => {
    ...
    const nodeC = [...plan.jobs.values()].find(n => n.producerId === 'c')!;

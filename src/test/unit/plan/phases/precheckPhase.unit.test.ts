...
suite('PrecheckPhaseExecutor', () => {
  test('returns success when no workSpec', async () => {
    const executor = new PrecheckPhaseExecutor({ getCopilotConfigDir: () => '/tmp' });
  ...
  test('returns success for shell spec with agent delegator on agent type', async () => {
    ...
    const executor = new PrecheckPhaseExecutor({ agentDelegator: delegator, getCopilotConfigDir: () => '/tmp' });

...
suite('DefaultJobExecutor', () => {
  ...
  suite('setStoragePath()', () => {
    test('stores storage path without creating logs directory', () => {
      const tmp = makeTmpDir();
      executor.setStoragePath(tmp);
      // Log files now go under plans/<planId>/specs/<nodeId>/current/
      // No longer creates a separate logs/ directory
      assert.ok(true); // Just verify no error
    });

    test('does not fail if storage dir already exists', () => {
      const tmp = makeTmpDir();
  ...
  suite('log()', () => {
    ...
    test('creates log file on disk when storagePath set', () => {
      ...
      // Log file now at plans/<planId>/specs/<nodeId>/current/execution.log
      const logFile = path.join(tmp, 'plans', 'p1', 'specs', 'n1', 'current', 'execution.log');
      assert.ok(fs.existsSync(logFile), 'Should have created a log file at new path');
    });
  ...
  suite('readLogsFromFile()', () => {
    ...
    test('returns error message when log file is unreadable', () => {
      ...
      // Log file now at plans/<planId>/specs/<nodeId>/current/execution.log
      const logFile = path.join(tmp, 'plans', 'p-err', 'specs', 'n-err', 'current', 'execution.log');
  ...
  suite('appendToLogFile error handling', () => {
    test('ignores file write errors silently', () => {
      ...
      // Log file now at plans/<planId>/specs/<nodeId>/current/execution.log
      const logFile = path.join(tmp, 'plans', 'p-wr', 'specs', 'n-wr', 'current', 'execution.log');
      assert.ok(fs.existsSync(logFile));
      fs.unlinkSync(logFile);
  ...
  suite('execute() - no work', function () {
  ...
  suite('execute() - with shell work', function () {
    this.timeout(30000); // These tests spawn real shell processes
    ...
    test('runs work command successfully', async function () {
      this.timeout(30000);
      const tmp = makeTmpDir();
      // Initialize git repo
      const { execSync } = require('child_process');
    ...
    test('runs postchecks after work', async function () {
      this.timeout(30000);
      const tmp = makeTmpDir();
      const { execSync } = require('child_process');
      try {

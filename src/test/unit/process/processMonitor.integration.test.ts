...
suite('ProcessMonitor', () => {
  ...
  suite('getSnapshot', () => {
    ...
    test('returns cached snapshot within TTL', async () => {
      ...
      const stubData: ProcessInfo[] = [{ pid: 1, parentPid: 0, name: 'test', cpu: 0, memory: 0 }];
      sinon.stub(cachedMonitor as any, 'getWindowsProcesses').resolves(stubData);
      sinon.stub(cachedMonitor as any, 'getUnixProcesses').resolves(stubData);

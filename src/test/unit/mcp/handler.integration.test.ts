...
suite('McpHandler', () => {
  ...
  setup(() => {
    ...
    handler = new McpHandler(stubRunner, '/workspace', {} as any, {} as any, undefined);
  });
  ...
  suite('tool routing', () => {
    ...
    test('routes get_copilot_job', async () => {
      const response = await handler.handleRequest(makeToolCall('get_copilot_job', {}));
      assert.ok(response.result);
    });

    test('routes list_copilot_jobs', async () => {
      const response = await handler.handleRequest(makeToolCall('list_copilot_jobs', {}));
      assert.ok(response.result);
    });

    test('routes retry_copilot_job', async () => {
      const response = await handler.handleRequest(makeToolCall('retry_copilot_job', {}));
      assert.ok(response.result);
    });

    test('routes force_fail_copilot_job', async () => {
      const response = await handler.handleRequest(makeToolCall('force_fail_copilot_job', {}));
      assert.ok(response.result);
    });

    test('routes get_copilot_job_failure_context', async () => {
      const response = await handler.handleRequest(makeToolCall('get_copilot_job_failure_context', {}));
      assert.ok(response.result);
    });
  });
});

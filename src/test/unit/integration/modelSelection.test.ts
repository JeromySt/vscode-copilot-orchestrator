...
suite('Model Selection Integration', () => {
  ...
  suite('Executor passes model to delegator', () => {
    // This test is skipped because it relies on spawn stubbing across module boundaries
    // which doesn't work reliably due to module caching. The model parameter is tested
    // through unit tests of CopilotCliRunner and buildCommand directly.
    test.skip('delegate receives model from options', async () => {
  ...
  suite('Token extraction from mock logs', () => {
    // These tests are skipped because they rely on complex integration between
    // AgentDelegator, CopilotCliRunner, and file system operations that don't
    // work reliably with spawn stubs. Token extraction is tested through
    // direct unit tests of the extractTokenUsage method.
    test.skip('extractTokenUsage parses prompt_tokens and completion_tokens', async () => {
      const cliCheckCore = require('../../../agent/cliCheckCore');
      sinon.stub(cliCheckCore, 'isCopilotCliAvailable').returns(true);
...
`;
    ...
    test.skip('extractTokenUsage handles input_tokens and output_tokens format', async () => {
      const cliCheckCore = require('../../../agent/cliCheckCore');
      sinon.stub(cliCheckCore, 'isCopilotCliAvailable').returns(true);
  ...
  suite('Invalid model logs warning', () => {
    // This test is skipped because it relies on model discovery spawn stubs
    // which don't work reliably across module boundaries due to caching.
    // The model validation warning is tested through direct unit tests.
    test.skip('delegator logs warning when unknown model is used', async () => {
